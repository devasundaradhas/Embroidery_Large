Public Class Color_Creation
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from Colour_Head where Colour_IdNo = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("select Colour_IdNo, Colour_Name from Colour_Head where Colour_IdNo <> 0 order by Colour_IdNo", con)
        ...
        With dgv_filter
            ...
            .Columns(1).HeaderText = "COLOUR NAME"
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select * from Colour_head a where Colour_idno = " & Str(Val(idno)), con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("Colour_IdNo").ToString
            txt_Name.Text = dt.Rows(0).Item("Colour_Name").ToString
        End If
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Colour_idno) from Colour_head Where Colour_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Colour_idno) from Colour_head Where Colour_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Colour_idno) from Colour_head Where Colour_idno > " & Str(Val(lbl_IdNo.Text)) & " and Colour_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Colour_idno) from Colour_head Where Colour_idno < " & Str(Val(lbl_IdNo.Text)) & " and Colour_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Colour_Head", "Colour_IdNo", "")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("select Colour_Name from Colour_Head order by Colour_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Find.DataSource = dt
        cbo_Find.DisplayMember = "Colour_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Colour_Head", "Colour_IdNo", "", trans)

                cmd.CommandText = "Insert into Colour_Head(Colour_IdNo, Colour_Name, Sur_Name) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "')"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Colour_Head set Colour_Name = '" & Trim(txt_Name.Text) & "', Sur_Name = '" & Trim(Sur) & "' where Colour_IdNo = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
            ...
            Common_Procedures.Master_Return.Master_Type = "COLOUR"
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_colour_head") > 0 Then
                MessageBox.Show("Duplicate Colour Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btn_Open_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Open.Click
        ...
        da = New SqlClient.SqlDataAdapter("select Colour_IdNo from Colour_Head where Colour_Name = '" & Trim(cbo_Find.Text) & "'", con)
        da.Fill(dt)
    ...
    Private Sub cbo_Find_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Find.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Find, Nothing, Nothing, "Colour_Head", "Colour_Name", "", "")
    ...
    Private Sub cbo_Find_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Find.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Find, Nothing, "Colour_Head", "Colour_Name", "", "")
    End Sub
