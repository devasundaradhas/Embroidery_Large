Public Class SalesReturn_Entry
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Try
            ...
            Da = New SqlClient.SqlDataAdapter("select SalesReturn_No from SalesReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and SalesReturn_Code = '" & Trim(RefCode) & "' and Entry_VAT_GST_Type <> 'GST'", con)
            Da.Fill(Dt)
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        Try
            ...
            Da = New SqlClient.SqlDataAdapter("select SalesReturn_No from SalesReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and SalesReturn_Code = '" & Trim(InvCode) & "' and Entry_VAT_GST_Type <> 'GST'", con)
            Da.Fill(Dt)
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    If Val(.Rows(i).Cells(4).Value) <> 0 Then
                        ...
                        If itm_id <> 0 Then
                            ...
                            cmd.CommandText = "Insert into SalesReturn_Details ( SalesReturn_Code,             Company_IdNo         ,               SalesReturn_No      ,         for_OrderBy        , SalesReturn_Date,          Ledger_IdNo    ,        Sl_No         ,          Item_IdNo      ,          Unit_IdNo      ,                  Serial_No             ,                 Noof_Items               ,                      Rate                ,                      Amount               ) " & _
                                                            "    Values ( '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_ReturnNo.Text) & "', " & Str(Val(vforOrdby)) & ",     @SalesDate  , " & Str(Val(led_id)) & ", " & Str(Val(Sno)) & ", " & Str(Val(itm_id)) & ", " & Str(Val(unt_id)) & ", '" & Trim(.Rows(i).Cells(3).Value) & "', " & Str(Val(.Rows(i).Cells(4).Value)) & ", " & Str(Val(.Rows(i).Cells(5).Value)) & ", " & Str(Val(.Rows(i).Cells(6).Value)) & " )"
                            cmd.ExecuteNonQuery()
