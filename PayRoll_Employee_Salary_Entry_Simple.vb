Public Class PayRoll_Employee_Salary_Entry_Simple
                ...
                With dgv_Details
                    ...
                    If dt2.Rows.Count > 0 Then
                        ...
                        For i = 0 To dt2.Rows.Count - 1
                            ...
                            .Rows(n).Cells(3).Value = Val(dt2.Rows(i).Item("No_Of_Attendance_Days").ToString)
        ...
        With dgv_Details
            ...
            If dt1.Rows.Count > 0 Then
                ...
                For i = 0 To dt1.Rows.Count - 1
                    ...
                    .Rows(n).Cells(3).Value = Val(wrk_dys)
                    If Val(.Rows(n).Cells(3).Value) = 0 Then .Rows(n).Cells(3).Value = ""
                    ...
                    .Rows(n).Cells(6).Value = Val(OT_Sal_Shft)
                    If Val(.Rows(n).Cells(6).Value) = 0 Then .Rows(n).Cells(6).Value = ""
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        cmd.CommandText = "select * from PayRoll_Salary_Head where Salary_Payment_Type_IdNo = " & Str(Val(SalPymtTyp_IdNo)) & " and Salary_Code <> '" & Trim(NewCode) & "' and ( (@SalaryFromDate Between From_Date and To_Date) or (@SalaryToDate Between From_Date and To_Date) )"
        Da = New SqlClient.SqlDataAdapter(cmd)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
        ...
        Try
            ...
            If New_Entry = True Then
                cmd.CommandText = "Insert into PayRoll_Salary_Head (     Salary_Code        ,               Company_IdNo       ,           Salary_No           ,                               for_OrderBy                              ,   Salary_Date ,       Salary_Payment_Type_IdNo   ,          Month_IdNo  ,  From_Date,  To_Date,             Total_Days              ,                  Festival_Days           ) " & _
                                    "     Values           ( '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ",  @SalaryDate  , " & Str(Val(SalPymtTyp_IdNo)) & ", " & Val(Mth_IDNo) & ", @FromDate , @ToDate , " & Str(Val(txt_TotalDays.Text)) & ",  " & Str(Val(txt_FestivalDays.Text)) & " ) "
                cmd.ExecuteNonQuery()
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(Emp_ID) <> 0 Then
                        ...
                        If Val(.Rows(i).Cells(14).Value) <> 0 Then
                            ...
                            End If
                            vVou_Amts = Math.Abs(Val(.Rows(i).Cells(14).Value)) & "|" & -1 * Math.Abs(Val(.Rows(i).Cells(14).Value))
