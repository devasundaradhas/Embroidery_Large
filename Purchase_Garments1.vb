Public Class Purchase_Garments1
    ...
    Private Pk_Condition As String = "PUGM1-"
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name as LedgerName from Purchase_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo where a.Purchase_Code = '" & Trim(NewCode) & "'", con)
            da1.Fill(dt1)
            ...
            If dt1.Rows.Count > 0 Then
                ...
                txt_TradeDiscPerc.Text = Format(Val(dt1.Rows(0).Item("TradeDiscount_Perc").ToString), "########0.00")
                txt_TradeDiscAmount.Text = Format(Val(dt1.Rows(0).Item("TradeDiscount_Amount").ToString), "########0.00")
                ...
                If dt2.Rows.Count > 0 Then
                    ...
                    For i = 0 To dt2.Rows.Count - 1
                        ...
                        dgv_Details.Rows(n).Cells(9).Value = dt2.Rows(i).Item("Purchase_Detail_SlNo").ToString
                        dgv_Details.Rows(n).Cells(7).Value = dt2.Rows(i).Item("Purchase_Order_Code").ToString
                        dgv_Details.Rows(n).Cells(8).Value = dt2.Rows(i).Item("Purchase_Order_Detail_SlNo").ToString
    ...
    Private Sub Invoice_Garments_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        da = New SqlClient.SqlDataAdapter("select distinct(Despatch_To) from Purchase_Head order by Despatch_To", con)
        da.Fill(dt5)
        cbo_Destination.DataSource = dt5
        cbo_Destination.DisplayMember = "Despatch_To"

        da = New SqlClient.SqlDataAdapter("select distinct(Booked_By) from Purchase_Head order by Booked_By", con)
        da.Fill(dt6)
        cbo_BookedBy.DataSource = dt6
        cbo_BookedBy.DisplayMember = "Booked_By"

        da = New SqlClient.SqlDataAdapter("select distinct(Document_Through) from Purchase_Head order by Document_Through", con)
        da.Fill(dt7)
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.CommandText = "Update Purchase_Order_Details Set Purchase_Items = a.Purchase_Items - b.Noof_Items from Purchase_Order_Details a, Purchase_Details b where b.Purchase_Code = '" & Trim(NewCode) & "' and b.Entry_Type = 'PO' and a.Purchase_Order_Code = b.Purchase_Order_Code and a.Purchase_Order_Detail_SlNo = b.Purchase_Order_Detail_SlNo"
            cmd.ExecuteNonQuery()
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            Else
                ...
                lbl_InvoiceNo.Text = Common_Procedures.get_MaxCode(con, "Purchase_Head", "Purchase_Code", "For_OrderBy", "", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
            ...
            With dgv_Details
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Val(dgv_Details.Rows(i).Cells(4).Value) <> 0 Then
                        ...
                        'cmd.CommandText = "Update Purchase_Details set Purchase_Date= @PurchaseDate, Entry_Type = '" & Trim(cbo_EntType.Text) & "', Ledger_IdNo = " & Str(Val(led_id)) & ",  Sl_No = " & Str(Val(Sno)) & ", Item_Idno = " & Str(Val(itm_id)) & ",Size_IdNo = " & Str(Val(Sz_id)) & " ,  Bags = " & Str(Val(.Rows(i).Cells(3).Value)) & ", Noof_Items = " & Str(Val(.Rows(i).Cells(4).Value)) & ", Unit_Idno = " & Val(unt_id) & ", Rate = " & Str(Val(.Rows(i).Cells(5).Value)) & ", Amount = " & Str(Val(.Rows(i).Cells(5).Value)) & ",Total_Amount = " & Str(Val(.Rows(i).Cells(6).Value)) & " ,   Purchase_Order_Code = '" & Trim(.Rows(i).Cells(7).Value) & "', Purchase_Order_Detail_SlNo = " & Str(Val(.Rows(i).Cells(8).Value)) & " Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Purchase_Code = '" & Trim(NewCode) & "' and Purchase_Detail_SlNo = " & Str(Val(.Rows(i).Cells(9).Value))
                        'nr = cmd.ExecuteNonQuery()

                        ' If nr = 0 Then
                        cmd.CommandText = "Insert into Purchase_Details ( Purchase_Code,            Company_IdNo          ,              Purchase_No             ,                                              for_OrderBy                   , Purchase_Date,          Ledger_IdNo    ,            SL_No     ,          Item_IdNo      ,          Unit_IdNo      ,            Size_IdNo   ,   Bags  ,             Noof_Items                 ,                 Rate                     ,                 Amount                   ,                    Total_Amount    ,                    Entry_Type ,                           Purchase_Order_Code ,                            Purchase_Order_Detail_SlNo ) " & _
                        ...
                        If Trim(UCase(cbo_EntType.Text)) = "PO" Then
                            ...
                            cmd.CommandText = "Update Purchase_Order_Details Set Purchase_Items = Purchase_Items + " & Str(Val(.Rows(i).Cells(4).Value)) & " where Purchase_Order_Code = '" & Trim(.Rows(i).Cells(7).Value) & "' and Purchase_Order_Detail_SlNo = " & Str(Val(.Rows(i).Cells(8).Value)) & " and Ledger_IdNo = " & Str(Val(led_id))
                            nr = cmd.ExecuteNonQuery()
                        ...
                        cmd.CommandText = "Insert into Item_Processing_Details (  Reference_Code      ,               Company_IdNo       ,            Reference_No           ,                                 for_OrderBy                                , Reference_Date,        Ledger_IdNo     ,            Party_Bill_No          ,           SL_No      ,           Item_IdNo     ,            Unit_IdNo    ,                    Quantity                             ,          Size_IdNo        ) " & _
                                                " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_InvoiceNo.Text))) & ",    @PurchaseDate, " & Str(Val(led_id)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(Sno)) & ", " & Str(Val(itm_id)) & ", " & Str(Val(unt_id)) & ", " & Str(Val(dgv_Details.Rows(i).Cells(4).Value)) & " ,   " & Str(Val(Sz_id)) & " ) "
                        cmd.ExecuteNonQuery()
    ...
    Private Sub cbo_ItemName_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_ItemName.LostFocus
        ...
        '        txt_Rate.Text = dt.Rows(0)("Purchase_Rate").ToString
        '    End If
        '    dt.Dispose()
        '    da.Dispose()
        'End If
    ...
    Private Sub cbo_Transport_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Transport.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Transport, cbo_Destination, cbo_ItemName, "Transport_Head", "Transport_Name", "", "(Transport_IdNo = 0)")
    End Sub

    Private Sub cbo_Transport_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Transport.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Transport, cbo_ItemName, "Transport_Head", "Transport_Name", "", "(Transport_IdNo = 0)")
    End Sub
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.* from Purchase_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo INNER JOIN Company_Head c ON a.Company_IdNo = c.Company_IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(NewCode) & "'", con)
            prn_HdDt = New DataTable
            da1.Fill(prn_HdDt)

            If prn_HdDt.Rows.Count > 0 Then

                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Size_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Size_Head c on a.size_idno = c.size_idno where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(NewCode) & "' Order by a.sl_no", con)
    ...
    Private Sub Printing_Format1_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 591, CurY, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 720, 140, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 620, 140, 0, 0, p1Font)
            CurY = TMargin + 180
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 590, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 720, 190, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 620, 190, 0, 0, pFont)
    ...
    Private Sub Printing_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Size_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Size_Head c on a.size_idno = c.size_idno where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(EntryCode) & "' Order by a.sl_no", con)
        da2.Fill(dt2)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + Cen1 + W1 + 25, CurY + 15, 0, 0, p1Font)
            ...
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + Cen1 + W1 + 10, CurY + 15, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + Cen1 + W1 + 25, CurY + 15, 0, 0, pFont)
    ...
    Private Sub Printing_Format3_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 620, CurY, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 720, 140, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Purchase_No").ToString, LMargin + 620, 140, 0, 0, p1Font)
            CurY = TMargin + 242 ' 235 ' 245
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 620, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 720, 190, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Purchase_Date").ToString), "dd-MM-yyyy"), LMargin + 620, 190, 0, 0, pFont)
    ...
    Private Sub btn_Selection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Selection.Click
        ...
        With dgv_Selection
            ...
            Da = New SqlClient.SqlDataAdapter("select a.*, b.Item_name, e.Size_Name, f.Noof_Items as Ent_Purchase_Quantity, f.Rate as Ent_Rate, f.Purchase_Detail_SlNo as Ent_Purchase_SlNo from Purchase_Order_Details a INNER JOIN Item_Head b ON a.Item_idno = b.Item_idno  LEFT OUTER JOIN Size_Head e ON a.Size_IdNo = e.Size_IdNo LEFT OUTER JOIN Purchase_Details F ON f.Purchase_Code = '" & Trim(NewCode) & "' and f.Entry_Type = '" & Trim(cbo_EntType.Text) & "' and a.Purchase_Order_Code = f.Purchase_Order_Code and a.Purchase_Order_Detail_SlNo = f.Purchase_Order_Detail_SlNo Where a.ledger_idno = " & Str(Val(LedIdNo)) & " and ( (a.Noof_Items  - a.Purchase_Items ) > 0 or f.Noof_Items > 0 ) Order by a.For_OrderBy, a.Purchase_Order_No, a.Purchase_Order_Detail_SlNo", con)
            Dt1 = New DataTable
            Da.Fill(Dt1)
            ...
            If Dt1.Rows.Count > 0 Then
                ...
                For i = 0 To Dt1.Rows.Count - 1
                    ...
                    If (Val(Dt1.Rows(i).Item("Noof_Items").ToString) - Val(Dt1.Rows(i).Item("Purchase_Items").ToString) + Ent_Qty) > 0 Then
                        ...
                        .Rows(n).Cells(1).Value = Dt1.Rows(i).Item("Purchase_Order_No").ToString
                        .Rows(n).Cells(2).Value = Dt1.Rows(i).Item("Item_name").ToString
                        .Rows(n).Cells(3).Value = Dt1.Rows(i).Item("Size_Name").ToString
                        .Rows(n).Cells(4).Value = (Val(Dt1.Rows(i).Item("Noof_Items").ToString) - Val(Dt1.Rows(i).Item("Purchase_Items").ToString) + Ent_Qty)
                        ...
                        End If
                        .Rows(n).Cells(7).Value = Dt1.Rows(i).Item("Purchase_Order_Code").ToString
                        .Rows(n).Cells(8).Value = Val(Dt1.Rows(i).Item("Purchase_Order_Detail_SlNo").ToString)
    ...
    Private Sub getItemDetails()
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select  Piece_Box ,Purchase_Rate from Item_Details where Item_IdNo = " & Common_Procedures.Item_NameToIdNo(con, Trim(cbo_ItemName.Text)) & " and  Size_IdNo =" & Common_Procedures.Size_NameToIdNo(con, Trim(cbo_Size.Text)), con)
            da1.Fill(dt1)

            If dt1.Rows.Count > 0 Then
                txt_Pc_Box.Text = Val(dt1.Rows(0).Item("Piece_Box").ToString)
                txt_Rate.Text = Val(dt1.Rows(0).Item("Purchase_Rate").ToString)
            End If
            dt1.Clear()
    ...
    Private Sub cbo_ItemName_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_ItemName.SelectedValueChanged
        getItemDetails()
    End Sub
    ...
    Private Sub cbo_Size_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_Size.SelectedValueChanged
        getItemDetails()
    End Sub
