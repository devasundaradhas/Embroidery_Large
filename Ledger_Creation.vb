...
Public Class Ledger_Creation
    ...
    Private WithEvents dgtxt_discountdetails As New DataGridViewTextBoxEditingControl
    ...
    Private Sub clear()
        ...
        txt_RatePer1000.Text = ""
        txt_Minimum_Pcs.Text = ""
        txt_MinimumAmount.Text = ""
        dgv_Details.Rows.Clear()
        dgv_DiscountDetails.Rows.Clear()
        txt_Remarks.Text = ""

        If Trim(Common_Procedures.settings.CustomerCode) = "1117" Then
            btn_LedgerSettings.Visible = True
        Else
            btn_LedgerSettings.Visible = False
        End If
    ...
    Private Sub Grid_Cell_DeSelect()
        ...
        If Not IsNothing(dgv_DiscountDetails.CurrentCell) Then dgv_DiscountDetails.CurrentCell.Selected = False
    End Sub
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select a.*,a.Ledger_IdNo, A.State_Idno , a.LedgerGroup_Idno , a.Ledger_Name, a.Ledger_MainName, a.Ledger_AlaisName, b.AccountsGroup_Name, c.Area_Name, a.Bill_Type, a.Ledger_Address1, a.Ledger_Address2, a.Ledger_Address3, a.Ledger_Address4, a.Ledger_EmailID, a.Ledger_PhoneNo, a.Ledger_TinNo, a.Ledger_CstNo, d.Price_List_Name,A.Agent_IdNo ,a.Owner_Name,A.Ledger_PanNo from ledger_head a LEFT OUTER JOIN Area_Head c ON a.Area_IdNo = c.Area_IdNo LEFT OUTER JOIN Price_List_Head d ON a.Price_List_IdNo = d.Price_List_IdNo , AccountsGroup_Head b where a.ledger_idno = " & Str(Val(idno)) & " and a.AccountsGroup_IdNo = b.AccountsGroup_IdNo", con)
        da.Fill(dt)
        ...
        If dt.Rows.Count > 0 Then
            ...
            txt_PanNo.Text = Trim(dt.Rows(0)("Ledger_PanNo").ToString)
            ...
            'If Check_BillToBill(dt.Rows(0).Item("Ledger_IdNo").ToString, dt.Rows(0)("Bill_Type").ToString) = True Then
            'cbo_BillType.Enabled = False
            'End If

            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1080" Then '----adhiwswara traders
                txt_RatePer1000.Text = Format(Val(dt.Rows(0)("Rate_For_1000").ToString), "############0")
            Else
                txt_RatePer1000.Text = dt.Rows(0)("Rate_For_1000").ToString
            End If


            txt_Minimum_Pcs.Text = dt.Rows(0)("Minimum_Pcs").ToString
            txt_MinimumAmount.Text = dt.Rows(0)("Minimum_Bill_Amount").ToString

            'If Not IsDBNull(dt.Rows(0)("Remarks")) Then
            '    txt_Remarks.Text = dt.Rows(0)("Remarks").ToString
            'End If

            If Not IsDBNull(dt.Rows(0)("Ledger_Legal_Name")) Then
                txt_LegalName.Text = dt.Rows(0)("Ledger_Legal_Name").ToString
            End If

            da = New SqlClient.SqlDataAdapter("select a.*, b.Machine_Name from LedgeR_Reading_Details a INNER JOIN Machine_Head b ON a.Machine_Idno = b.Machine_IdNo where a.Ledger_IdNo = " & Val(idno), con)
            da.Fill(Dt2)
            ...
            da = New SqlClient.SqlDataAdapter("select a.* from ledger_head a  where a.ledger_idno = " & Str(Val(idno)) & "", con)
            da.Fill(dt3)

            If dt3.Rows.Count > 0 Then

                txt_RentMachine.Text = dt3.Rows(0)("Rent_Machine").ToString
                txt_RateExtraCopy.Text = dt3.Rows(0)("Rate_Extra_Copy").ToString
                txt_FreeCopiesMachine.Text = dt3.Rows(0)("Free_Copies_Machine").ToString
                txt_TotalMachine.Text = dt3.Rows(0)("Total_Machine").ToString
            End If
            dt3.Clear()
            dt3.Dispose()
            ...
            da = New SqlClient.SqlDataAdapter("select a.*, b.ItemGroup_Name from Ledger_DiscountDetails a INNER JOIN ItemGroup_Head b ON a.ItemGroup_IdNo = b.ItemGroup_IdNo where a.Ledger_IdNo = " & Val(idno), con)
            da.Fill(Dt2)
            dgv_DiscountDetails.Rows.Clear()
            SNo = 0
            ...
            If Dt2.Rows.Count > 0 Then
                ...
                For i = 0 To Dt2.Rows.Count - 1
                    ...
                    n = dgv_DiscountDetails.Rows.Add()

                    SNo = SNo + 1
                    dgv_DiscountDetails.Rows(n).Cells(0).Value = Val(SNo)
                    dgv_DiscountDetails.Rows(n).Cells(1).Value = Dt2.Rows(i).Item("ItemGroup_Name").ToString
                    dgv_DiscountDetails.Rows(n).Cells(2).Value = Val(Dt2.Rows(i).Item("Discount_Percentage").ToString)

                Next i

                For i = 0 To dgv_DiscountDetails.RowCount - 1
                    dgv_DiscountDetails.Rows(i).Cells(0).Value = Val(i) + 1
                Next
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.CommandText = "delete from Ledger_DiscountDetails where Ledger_idno = " & Str(Val(lbl_IdNo.Text))
            cmd.ExecuteNonQuery()
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                cmd.CommandText = "Insert into ledger_head(Ledger_IdNo, Ledger_Name, Sur_Name, Ledger_MainName, Ledger_AlaisName, Area_IdNo, AccountsGroup_IdNo, Parent_Code, Bill_Type, Ledger_Address1, Ledger_Address2, Ledger_Address3, Ledger_Address4,Ledger_EmailID , Ledger_PhoneNo,  Ledger_Type, Price_List_IdNo,Rent_Machine,Free_Copies_Machine,Rate_Extra_Copy,Total_Machine , State_Idno ,LedgerGroup_Idno ,Agent_idNo,Owner_Name ,Ledger_PanNo,Ledger_GSTinNo ,Rate_For_1000 ,Minimum_Bill_Amount ,Minimum_Pcs,Ledger_Legal_Name) Values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(LedName) & "', '" & Trim(SurName) & "', '" & Trim(txt_Name.Text) & "', '" & Trim(txt_AlaisName.Text) & "', " & Str(Val(ar_idno)) & ", " & Str(Val(acgrp_idno)) & ", '" & Trim(Parnt_CD) & "', '" & Trim(cbo_BillType.Text) & "', '" & Trim(txt_Address1.Text) & "', '" & Trim(txt_Address2.Text) & "', '" & Trim(txt_Address3.Text) & "', '" & Trim(txt_Address4.Text) & "','" & Trim(txt_EmailID.Text) & "' , '" & Trim(txt_PhoneNo.Text) & "',  '', " & Str(Val(PrcLst_idno)) & "," & Str(Val(txt_RentMachine.Text)) & "  ," & Str(Val(txt_FreeCopiesMachine.Text)) & " , " & Str(Val(txt_RateExtraCopy.Text)) & "," & Str(Val(txt_TotalMachine.Text)) & " ,  " & Str(Val(State_idno)) & "  , " & Str(Val(Grp_idno)) & "  , " & Str(Val(Agnt_idno)) & " ,'" & Trim(txt_OwnerName.Text) & "','" & Trim(txt_PanNo.Text) & "','" & Trim(txt_GSTIN_No.Text) & "' ," & Val(txt_RatePer1000.Text) & " , " & Val(txt_MinimumAmount.Text) & "," & Val(txt_Minimum_Pcs.Text) & ",'" & txt_LegalName.Text & "')"

            Else

                cmd.CommandText = "Update ledger_head Set Ledger_Name = '" & Trim(LedName) & "', Sur_Name = '" & Trim(SurName) & "', Ledger_MainName = '" & Trim(txt_Name.Text) & "', Ledger_AlaisName = '" & Trim(txt_AlaisName.Text) & "', State_Idno = " & Str(Val(State_idno)) & " , LedgerGroup_Idno = " & Str(Val(Grp_idno)) & " , Area_IdNo = " & Str(Val(ar_idno)) & ", AccountsGroup_IdNo = " & Str(Val(acgrp_idno)) & ", Parent_Code = '" & Trim(Parnt_CD) & "', Bill_Type = '" & Trim(cbo_BillType.Text) & "', Ledger_Address1 = '" & Trim(txt_Address1.Text) & "', Ledger_Address2 = '" & Trim(txt_Address2.Text) & "', Ledger_Address3 = '" & Trim(txt_Address3.Text) & "', Ledger_Address4 = '" & Trim(txt_Address4.Text) & "',Ledger_EmailID = '" & Trim(txt_EmailID.Text) & "' , Ledger_PhoneNo = '" & Trim(txt_PhoneNo.Text) & "',Price_List_IdNo = " & Str(Val(PrcLst_idno)) & ",Rent_Machine = " & Str(Val(txt_RentMachine.Text)) & "  , Free_Copies_Machine = " & Str(Val(txt_FreeCopiesMachine.Text)) & " , Rate_Extra_Copy = " & Str(Val(txt_RateExtraCopy.Text)) & " , Total_Machine = " & Str(Val(txt_TotalMachine.Text)) & " , Agent_idNo = " & Str(Val(Agnt_idno)) & ",Owner_Name = '" & Trim(txt_OwnerName.Text) & "',Ledger_PanNo =   '" & Trim(txt_PanNo.Text) & "',Ledger_GSTinNo='" & Trim(txt_GSTIN_No.Text) & "' ,Rate_For_1000 = " & Val(txt_RatePer1000.Text) & " ,Minimum_Bill_Amount = " & Val(txt_MinimumAmount.Text) & " ,Minimum_Pcs = " & Val(txt_Minimum_Pcs.Text) & ",Ledger_Legal_Name = '" & txt_LegalName.Text & "'  where Ledger_IdNo = " & Str(Val(lbl_IdNo.Text))
            ...
            For i = 0 To UBound(PhAr)
                If Trim(PhAr(i)) <> "" Then
                    ...
                    LedPhNo = Trim(PhAr(i))
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, " ", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "-", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "_", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "+", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "/", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "\", "")
                    LedPhNo = Microsoft.VisualBasic.Replace(LedPhNo, "*", "")
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    Mac_id = Common_Procedures.Machine_NameToIdNo(con, .Rows(i).Cells(1).Value, trans)
                    If Val(Mac_id) <> 0 Then
                        Sno = Sno + 1
                        cmd.CommandText = "Insert into Ledger_Reading_Details(Ledger_IdNo, sl_no, Machine_IdNo, Opening_Reading ) values (" & Str(Val(lbl_IdNo.Text)) & ", " & Str(Val(Sno)) & ", " & Str(Val(Mac_id)) & ", " & Str(Val(.Rows(i).Cells(2).Value)) & " )"
                        cmd.ExecuteNonQuery()
                    End If
                Next
            ...
            cmd.ExecuteNonQuery()
            Mac_id = 0
            With dgv_DiscountDetails
                Sno = 0
                For i = 0 To .RowCount - 1
                    Mac_id = Common_Procedures.ItemGroup_NameToIdNo(con, .Rows(i).Cells(1).Value, trans)
                    If Val(Mac_id) <> 0 Then
                        Sno = Sno + 1
                        cmd.CommandText = "Insert into Ledger_DiscountDetails(Ledger_IdNo, sl_no, ItemGroup_IdNo, Discount_Percentage ) values (" & Str(Val(lbl_IdNo.Text)) & ", " & Str(Val(Sno)) & ", " & Str(Val(Mac_id)) & ", " & Str(Val(.Rows(i).Cells(2).Value)) & " )"
                        cmd.ExecuteNonQuery()
                    End If
                Next
    ...
    Private Sub Ledger_Creation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Trim(Common_Procedures.settings.CustomerCode) = "1053" Then
            btn_LedgerSettings.Visible = True
        Else
            btn_LedgerSettings.Visible = False
        End If

        If Trim(Common_Procedures.settings.CustomerCode) = "1085" Then

            lbl_Price_Agent.Text = "Agent"
            cbo_PriceListName.Visible = False
            cbo_Agent.Visible = True
        Else

            cbo_PriceListName.Visible = True
            cbo_Agent.Visible = False
        End If
        If Trim(Common_Procedures.settings.CustomerCode) = "1219" Then ' VEL ENTERPRISES
            btn_DiscountDetails.Visible = True
        End If
        ...
        'lbl_TinNo.Text = "Tin No"
        'lbl_CstNo.Text = "Cst No"
        'If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1016" Then '---- Rajendra Textiles (Somanur)
        '    lbl_TinNo.Text = "Ends1"
        '    lbl_CstNo.Text = "Ends2"
        'End If

        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1022" Then '---- gANESH(Somanur)
            lbl_Price_Agent.Visible = False
            cbo_PriceListName.Visible = False
        End If

        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1117" Then '---- SRI LAKSHMI DESIGN
            lbl_RatePer1000.Visible = True
            lbl_MinimumAmt.Visible = True
            txt_RatePer1000.Visible = True
            txt_MinimumAmount.Visible = True
            txt_Minimum_Pcs.Visible = True
            lbl_minimumPcs.Visible = True
        End If
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1080" Then '----adhiwswara traders
            lbl_RatePer1000.Visible = True
            txt_RatePer1000.Visible = True
            lbl_RatePer1000.Text = "Bill Due Days"
        End If
        ...
        grp_Settings.Left = grp_Back.Left
        grp_Settings.Top = grp_Back.Top + 100
        grp_Settings.Width = grp_Back.Width
        grp_Settings.Height = grp_Back.Height - 100
        ...
        pnl_DiscountDetails.Visible = False
        pnl_DiscountDetails.Left = (Me.Width - pnl_DiscountDetails.Width) / 2
        pnl_DiscountDetails.Top = (Me.Height - pnl_DiscountDetails.Height) / 2
        pnl_DiscountDetails.BringToFront()
        ...
        AddHandler txt_RatePer1000.GotFocus, AddressOf ControlGotFocus
        AddHandler txt_MinimumAmount.GotFocus, AddressOf ControlGotFocus
        AddHandler txt_Minimum_Pcs.GotFocus, AddressOf ControlGotFocus
        AddHandler txt_Remarks.GotFocus, AddressOf ControlGotFocus
        AddHandler cbo_ItemGroupName.GotFocus, AddressOf ControlGotFocus
        'AddHandler txt_GSTP_CA_Email_Id.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler txt_RatePer1000.LostFocus, AddressOf ControlLostFocus
        AddHandler txt_MinimumAmount.LostFocus, AddressOf ControlLostFocus
        AddHandler txt_Minimum_Pcs.LostFocus, AddressOf ControlLostFocus
        AddHandler txt_Remarks.LostFocus, AddressOf ControlLostFocus
        AddHandler cbo_ItemGroupName.LostFocus, AddressOf ControlLostFocus
        'AddHandler txt_GSTP_CA_Email_Id.LostFocus, AddressOf ControlLostFocus
    ...
    Private Sub txt_TinNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        If Asc(e.KeyChar) = 13 Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub txt_TinNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    End Sub
    ...
    '    'Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Agent, txt_CstNo, txt_OwnerName, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'AGENT') ", "(Ledger_IdNo = 0)")
    ...
    '    Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Agent, txt_OwnerName, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'AGENT') ", "(Ledger_IdNo = 0)")
    ...
    Private Sub txt_CstNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    ...
    Private Sub txt_CstNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
    ...
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        ...
        If ActiveControl.Name = dgv_Details.Name Or ActiveControl.Name = dgv_DiscountDetails.Name Or TypeOf ActiveControl Is DataGridViewTextBoxEditingControl Then
            ...
            ElseIf ActiveControl.Name = dgv_DiscountDetails.Name Then
                dgv1 = dgv_DiscountDetails

            ElseIf dgv_DiscountDetails.IsCurrentRowDirty = True Then
                dgv1 = dgv_DiscountDetails
            ElseIf pnl_DiscountDetails.Visible = True Then
                dgv1 = dgv_DiscountDetails
            ...
            With dgv1
                ...
                ElseIf dgv1.Name = dgv_DiscountDetails.Name Then

                    If keyData = Keys.Enter Or keyData = Keys.Down Then

                        If .CurrentCell.ColumnIndex >= .ColumnCount - 1 Then
                            If .CurrentCell.RowIndex = .RowCount - 1 Then
                                lbl_DiscountClose.Focus()
                            Else
                                .CurrentCell = .Rows(.CurrentCell.RowIndex + 1).Cells(1)
    ...
    Private Sub btn_MachineDetails_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_LedgerSettings.Click

        'grp_Back.Enabled = False
        'pnl_Reading.Visible = True
        'If txt_RentMachine.Visible And txt_RentMachine.Enabled Then txt_RentMachine.Focus()

        grp_Settings.Visible = True
        grp_Settings.BringToFront()
        txt_Remarks.Focus()
        grp_Back.Enabled = False
    ...
    Private Sub btn_SendSMSAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_SendSMSAll.Click
        Dim smstxt As String = ""
        Dim PhNo As String = ""
        Dim SMS_SenderID As String = ""
        Dim SMS_Key As String = ""
    ...
    Private Function Check_BillToBill(ByVal Ledger_Id As Integer, ByVal Bill_Type As String) As Boolean
        Dim da As New SqlClient.SqlDataAdapter
        Dim dt As New DataTable
        ...
        Try
            ...
            If Bill_Type = "BILL TO BILL" Then

                da = New SqlClient.SqlDataAdapter("select * from Voucher_Bill_Details Where  Ledger_IdNo  = " & Val(Ledger_Id), con)
                da.Fill(dt)
                If dt.Rows.Count > 0 Then
        ...
        Check_BillToBill = Sts
    End Function


    Private Sub txt_GSTIN_No_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_GSTIN_No.KeyDown
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    End Sub
    ...
    Private Sub txt_TinNo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)

    End Sub

    Private Sub txt_PanNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_PanNo.KeyDown
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    ...
    Private Sub txt_RatePer1000_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_RatePer1000.KeyDown
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    End Sub

    Private Sub txt_RatePer1000_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_RatePer1000.KeyPress
        If Asc(e.KeyChar) = 13 Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub txt_RatePer1000_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_RatePer1000.TextChanged

    End Sub

    Private Sub txt_MinimumAmount_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_MinimumAmount.KeyDown
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
    End Sub

    Private Sub txt_MinimumAmount_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_MinimumAmount.KeyPress
        If Asc(e.KeyChar) = 13 Then SendKeys.Send("{TAB}")
    End Sub


    Private Sub btn_DiscountDetails_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_DiscountDetails.Click
        pnl_DiscountDetails.Visible = True
        grp_Back.Enabled = False

        dgv_DiscountDetails.Focus()
        dgv_DiscountDetails.CurrentCell = dgv_DiscountDetails.Rows(0).Cells(1)
        dgv_DiscountDetails.CurrentCell.Selected = True

    End Sub

    Private Sub lbl_DiscountClose_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbl_DiscountClose.Click
        pnl_DiscountDetails.Visible = False
        grp_Back.Enabled = True
    End Sub

    Private Sub dgv_DiscountDetails_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_DiscountDetails.CellEnter
        Dim Da As New SqlClient.SqlDataAdapter
        Dim Dt1 As New DataTable
        Dim Dt2 As New DataTable
        Dim rect As Rectangle
        ...
        With dgv_DiscountDetails
            ...
            If e.ColumnIndex = 1 Then
                ...
                If cbo_ItemGroupName.Visible = False Or Val(cbo_ItemGroupName.Tag) <> e.RowIndex Then

                    cbo_ItemGroupName.Tag = -1
                    Da = New SqlClient.SqlDataAdapter("select ItemGroup_Name from ItemGroup_Head order by ItemGroup_Name", con)
                    Dt1 = New DataTable
                    Da.Fill(Dt1)
                    cbo_ItemGroupName.DataSource = Dt1
                    cbo_ItemGroupName.DisplayMember = "ItemGroup_Name"

                    rect = .GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, False)

                    cbo_ItemGroupName.Left = .Left + rect.Left
                    cbo_ItemGroupName.Top = .Top + rect.Top

                    cbo_ItemGroupName.Width = rect.Width
                    cbo_ItemGroupName.Height = rect.Height
                    cbo_ItemGroupName.Text = .CurrentCell.Value


                    cbo_ItemGroupName.Tag = Val(e.RowIndex)
                    cbo_ItemGroupName.Visible = True

                    cbo_ItemGroupName.BringToFront()
    ...
    Private Sub cbo_ItemGroupName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_ItemGroupName.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_ItemGroupName, Nothing, Nothing, "ItemGroup_Head", "ItemGroup_Name", "", "(ItemGroup_IdNo = 0)")

        With dgv_DiscountDetails

            If (e.KeyValue = 38 And cbo_ItemGroupName.DroppedDown = False) Or (e.Control = True And e.KeyValue = 38) Then
            ...
            If (e.KeyValue = 40 And cbo_ItemGroupName.DroppedDown = False) Or (e.Control = True And e.KeyValue = 40) Then
    ...
    Private Sub cbo_ItemGroupName_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_ItemGroupName.KeyPress
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_ItemGroupName, Nothing, "ItemGroup_Head", "ItemGroup_Name", "", "(ItemGroup_IdNo = 0)")
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            With dgv_DiscountDetails
                ...
                .Focus()
                .Rows(.CurrentCell.RowIndex).Cells.Item(1).Value = Trim(cbo_ItemGroupName.Text)
                If .CurrentCell.RowIndex = .RowCount - 1 And .CurrentCell.ColumnIndex >= 1 And Trim(.CurrentRow.Cells(1).Value) = "" Then
    ...
    Private Sub cbo_ItemName_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_ItemGroupName.KeyUp
        ...
        If e.Control = False And e.KeyValue = 17 And vcbo_KeyDwnVal = e.KeyValue Then
            ...
            Common_Procedures.Master_Return.Form_Name = Me.Name
            Common_Procedures.Master_Return.Control_Name = cbo_ItemGroupName.Name
            Common_Procedures.Master_Return.Return_Value = ""
            Common_Procedures.Master_Return.Master_Type = ""
    ...
    Private Sub dgtxt_discountdetails_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles dgtxt_discountdetails.KeyPress
    ...
    End Sub
    Private Sub dgv_DiscountDetails_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles dgv_DiscountDetails.RowsAdded
    ...
    Private Sub dgv_DiscountDetails_EditingControlShowing(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles dgv_DiscountDetails.EditingControlShowing
        dgtxt_discountdetails = Nothing
        'If dgv_DiscountDetails.CurrentCell.ColumnIndex > 2 Then
        dgtxt_discountdetails = CType(dgv_DiscountDetails.EditingControl, DataGridViewTextBoxEditingControl)
        'End If
    End Sub

    Private Sub dgtxt_discountdetails_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgtxt_discountdetails.Enter
        dgv_DiscountDetails.EditingControl.BackColor = Color.Lime
        dgv_DiscountDetails.EditingControl.ForeColor = Color.Blue
        dgtxt_discountdetails.SelectAll()
    End Sub
    ...
    Private Sub cbo_ItemGroupName_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_ItemGroupName.LostFocus
        If Trim(cbo_ItemGroupName.Text) <> "" Then
            dgv_DiscountDetails.Rows(dgv_DiscountDetails.CurrentCell.RowIndex).Cells(dgv_DiscountDetails.CurrentCell.ColumnIndex).Value = Trim(cbo_ItemGroupName.Text)
        End If
    End Sub



    Private Sub txt_Minimum_Pcs_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txt_Minimum_Pcs.KeyPress
        If Asc(e.KeyChar) = 13 Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub btn_CloseSettings_Click(sender As Object, e As EventArgs) Handles btn_CloseSettings.Click
    ...
    Private Sub txt_Remarks_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_Remarks.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            btn_CloseSettings_Click(sender, e)
        End If

    End Sub


    Private Sub txt_GSTIN_No_Enter(sender As Object, e As EventArgs) Handles txt_GSTIN_No.Enter
        txt_GSTIN_No.Tag = txt_GSTIN_No.Text
    End Sub

    Private Sub txt_GSTIN_No_Leave(sender As Object, e As EventArgs) Handles txt_GSTIN_No.Leave
        If txt_GSTIN_No.Tag <> txt_GSTIN_No.Text Then
            chk_GSTIN_Verified.Enabled = False
            chk_GSTIN_Verified.Checked = False
        End If
    End Sub
    ...
    Private Sub btn_Verify_Click(sender As Object, e As EventArgs) Handles btn_Verify.Click
        ...
        Dim STCODE As String = Common_Procedures.get_FieldValue(con, "State_Head", "State_Code", "State_Name = '" & Cbo_State.Text & "'")
        Dim G As New GST_UTILITY

        Dim GSTIN_INFO() As String = Split((G.Get_GSTIN_Reg_Info(txt_GSTIN_No.Text, STCODE)), "@#$")

        If UBound(GSTIN_INFO) < 1 Then
            MessageBox.Show("The GSTIN is either Invalid or Validation could not be done .", "Invalid GSTIN / Failed Validation", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Else
            If LCase(Trim(GSTIN_INFO(0))) = "invalid gstin" Then
                chk_GSTIN_Verified.Enabled = False
                chk_GSTIN_Verified.Checked = False
                MessageBox.Show(GSTIN_INFO(0) + "," + GSTIN_INFO(1), GSTIN_INFO(0), MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
            If LCase(Trim(GSTIN_INFO(0))) = "valid gstin" Then

                Dim cmpname As String = ""
                Dim jResults As Newtonsoft.Json.Linq.JObject = Newtonsoft.Json.Linq.JObject.Parse(GSTIN_INFO(1))
                Dim tradename As [String] = jResults("tradeNam")
                Dim legalname As [String] = jResults("lgnm")

                If Trim(UCase(tradename)) = Trim(UCase(txt_Name.Text)) Or Trim(UCase(legalname)) = Trim(UCase(txt_Name.Text)) Then

                    chk_GSTIN_Verified.Enabled = False
                    chk_GSTIN_Verified.Checked = True

                    MessageBox.Show("The GSTIN is verified . The Legal name / Trade name provided matches with GST registration information. ", "GSTIN Verified", MessageBoxButtons.OK, MessageBoxIcon.Information)
                ...
                Else
                    ...
                    MessageBox.Show("The Trade name returned for this GSTIN is " & tradename & " and Legalname returned for this GSTIN is " & legalname &
                                    ". You may manually mark this GSTIN as verified.", "GSTIN Verified", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub btn_Extract_Click(sender As Object, e As EventArgs) Handles btn_Extract.Click
        ...
        If MessageBox.Show("All Existing Data will be over written. Continue Anyway ? ", "Conform Action ! ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
            Exit Sub
        End If

        Dim STCODE As String = Common_Procedures.State_NameToIdNo(con, Cbo_State.Text)
        Dim G As New GST_UTILITY
        ...
        Else
            ...
            If LCase(Trim(GSTIN_INFO(0))) = "valid gstin" Then
                ...
                Dim dno As String = jResults("pradr")("addr")("bno")
                Dim localarea As String = jResults("pradr")("addr")("bnm")
                Dim street As String = jResults("pradr")("addr")("st")
                Dim locality As String = jResults("pradr")("addr")("loc")

                Dim town As String = jResults("pradr")("addr")("city")
                Dim district As String = jResults("pradr")("addr")("dst")
                Dim pincode As String = jResults("pradr")("addr")("pncd")

                chk_GSTIN_Verified.Enabled = False
                chk_GSTIN_Verified.Checked = True

                txt_Name.Text = tradename
                txt_LegalName.Text = legalname

                If Len(Trim(dno)) > 0 Then
                    txt_Address1.Text = dno
                End If

                If Len(Trim(localarea)) > 0 Then
                    If Len(Trim(txt_Address1.Text)) > 0 Then
                        txt_Address1.Text = txt_Address1.Text + ","
                    End If
                    txt_Address1.Text = txt_Address1.Text + localarea
                End If
                ...
                If Len(Trim(locality)) > 0 Then
                    If Len(Trim(ADD2.Text)) > 0 Then
                        ADD2.Text = ADD2.Text + ","
                    End If
                    ADD2.Text = ADD2.Text + locality
                End If

                If Len(Trim(town)) > 0 Then
                    txt_Address3.Text = town
                Else
                    txt_Address3.Text = district
                End If

                txt_Address4.Text = pincode
    ...
    Private Sub txt_LegalName_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_LegalName.KeyDown
        If e.KeyValue = 38 Then SendKeys.Send("+{TAB}")
        If e.KeyValue = 40 Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub txt_LegalName_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_LegalName.KeyPress
        If Asc(e.KeyChar) = 13 Then SendKeys.Send("{TAB}")
    End Sub
