...
Public Class Invoice_Embroidery_Pcs
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Trim(dgv_Details.Rows(i).Cells(1).Value) <> "" Or Val(dgv_Details.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        If UCase(Trim(Trim(.Rows(i).Cells(3).Value))) <> "ALL" Then
                            ...
                            cmd.CommandText = "Insert into Invoice_DC_Details ( Sales_Code,            Sales_DC_Code         ,Job_No  ,UID , Colour_IdNo) Values ('" & NewCode & "', '" & Trim(.Rows(i).Cells(3).Value) & "' , '" & Trim(.Rows(i).Cells(24).Value) & "' , '" & Trim(.Rows(i).Cells(2).Value) & "'," & clr_idno.ToString & ")"
                            cmd.ExecuteNonQuery()

                        Else

                            For j As Integer = 0 To Split(.Rows(i).Cells(23).Value, "$$$").GetUpperBound(0)
                                cmd.CommandText = "Insert into Invoice_DC_Details ( Sales_Code,            Sales_DC_Code         ,Job_No     ,UID, Colour_IdNo) Values ('" & NewCode & "', '" & Trim(Split(.Rows(i).Cells(23).Value, "$$$")(j)) & "' , '" & Trim(.Rows(i).Cells(24).Value) & "' , '" & Trim(.Rows(i).Cells(2).Value) & "'," & clr_idno.ToString & ")"
                                cmd.ExecuteNonQuery()
                            Next
    ...
    Private Sub dgv_Details_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgv_Details.DoubleClick
        ...
        If pnl_InputDetails.Enabled = True And txt_SlNo.Enabled = True Then
            ...
            If Trim(dgv_Details.CurrentRow.Cells(1).Value) <> "" Then
                ...
                'lbl_OrderNo.Text = Trim(dgv_Details.CurrentRow.Cells(2).Value)
        ...
        Try
            ...
            Try
                ...
                If prn_HdDt.Rows.Count > 0 Then
                    ...
                    da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, b.Item_Name_tamil, c.Unit_Name,OP.Order_No as OrdNo from Sales_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Unit_Head c on a.unit_idno = c.unit_idno  LEFT OUTER JOIN Order_Program_Head OP on " &
...
LOOP1:
    ...
    Private Sub Printing_Format2_GST(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClArr(1) = 25 : ClArr(2) = 110 : ClArr(3) = 280 : ClArr(4) = 60 : ClArr(5) = 0 : ClArr(6) = 0 : ClArr(7) = 85 : ClArr(8) = 80
        ...
        If prn_HdDt.Rows.Count > 0 Then
            ...
            If prn_DetMxIndx > 0 Then
                ...
                Do While DetIndx <= prn_DetMxIndx
                    ...
                    Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 4), LMargin + ClArr(1) + 5, CurY, 0, 0, p1Font)

                    If Trim(prn_DetAr(DetIndx, 11)) <> "" And Trim(prn_DetAr(DetIndx, 10)) = "SERIALNO" Then

                        CurY = CurY - 3
                        p1Font = New Font("Calibri", 8, FontStyle.Regular)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 11), LMargin + ClArr(1) + ClArr(2) + 5, CurY, 0, 0, p1Font)
                        ...
                        For Rw = 0 To 10
                            If Trim(prn_DetAr(DetIndx, 4)) <> "" Then
                                If Trim(Dc_No(Rw)) <> "" Then
                                    CurY1 = CurY1 + TxtHgt
                                    Common_Procedures.Print_To_PrintDocument(e, Trim(Dc_No(Rw)), LMargin + ClArr(1) + 5, CurY1, 0, 0, p1Font)
                                End If
                            End If
                        Next

                        Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 3), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 5, CurY, 0, 0, p1Font)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 8), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 5, CurY, 1, 0, p1Font)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 7), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) + ClArr(8) - 5, CurY, 1, 0, p1Font)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetAr(DetIndx, 9), PageWidth - 10, CurY, 1, 0, p1Font)
    ...
    Private Sub Printing_Format2_GST_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        Try
            ...
            If Trim(Common_Procedures.settings.CustomerCode) = "5010" Then
                ...
                '    e.Graphics.DrawImage(DirectCast(Global.Billing.My.Resources.Resources.FWC_LOGO, Drawing.Image), LMargin + 24, CurY + 10, 100, 100)

                '    p1Font = New Font("Arial Narrow", 9, FontStyle.Bold)

                '    strWidth = e.Graphics.MeasureString("(EMBROIDERY DIVISION)", p1Font).Width
                '    Common_Procedures.Print_To_PrintDocument(e, """The Thread Art Studio""", LMargin + 74 - (strWidth / 2), CurY + 115, 2, strWidth, p1Font, Brushes.Black)
            ...
            If Common_Procedures.settings.CustomerCode = "5022" Then
                CurY = CurY + TxtHgt
                'Common_Procedures.Print_To_PrintDocument(e, Cmp_PAN, PageWidth - 145, CurY + 95, 0, strWidth, p1Font, Brushes.Black)
                'Common_Procedures.Print_To_PrintDocument(e, Cmp_ESINo, PageWidth - 145, CurY + 110, 0, strWidth, p1Font, Brushes.Black)
                Common_Procedures.Print_To_PrintDocument(e, Cmp_GSTIN_No, LMargin, CurY - 10, 2, PrintWidth - 20, pFont, Brushes.Green)
            End If
            ...
            If Not IsDBNull(prn_HdDt.Rows(0).Item("Party_Ref_No")) Then
                If Len(Trim(prn_HdDt.Rows(0).Item("Party_Ref_No"))) > 0 Then
                    ...
                    Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + Cen1 + W1 + 10, BlockInvNoY, 0, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Trim(prn_HdDt.Rows(0).Item("Party_Ref_No")), LMargin + Cen1 + W1 + 25, BlockInvNoY, 0, 0, pFont)
                    BlockInvNoY = BlockInvNoY + TxtHgt + 10
                End If
...
A:
            ...
            Common_Procedures.Print_To_PrintDocument(e, "DC.NO/DATE", LMargin + ClAr(1), CurY + 5, 2, ClAr(2), p1Font, Brushes.Black)
            Common_Procedures.Print_To_PrintDocument(e, "PARTICULARS", LMargin + ClAr(1) + ClAr(2), CurY + 5, 2, ClAr(3), p1Font, Brushes.Black)
            ...
            Common_Procedures.Print_To_PrintDocument(e, "QUANTITY", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6), CurY + 5, 2, ClAr(7), p1Font, Brushes.Black)
            Common_Procedures.Print_To_PrintDocument(e, "RATE", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7), CurY + 5, 2, ClAr(8), p1Font, Brushes.Black)
            Common_Procedures.Print_To_PrintDocument(e, "AMOUNT", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8), CurY + 5, 2, ClAr(9), p1Font, Brushes.Black)
    ...
    Private Sub Printing_Format2_GST_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal Cmp_Name As String, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean)
        ...
        Try
            ...
            'If is_LastPage = True Then
            Common_Procedures.Print_To_PrintDocument(e, Val(prn_HdDt.Rows(0).Item("Total_Qty").ToString), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) - 10, CurY, 1, 0, p1Font)
            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("SubTotal_Amount").ToString), "########0.00"), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font)
            'End If
            ...
            If Val(prn_HdDt.Rows(0).Item("CGst_Amount").ToString) <> 0 Then
                ...
                'If is_LastPage = True Then
                If vTaxPerc <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "CGST @ " & Trim(Val(vTaxPerc)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "CGST ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                End If
                Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("CGst_Amount").ToString), "########0.00"), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font)
                'End If
            End If
            ...
            If Val(prn_HdDt.Rows(0).Item("SGst_Amount").ToString) <> 0 Then
                ...
                'If is_LastPage = True Then
                If vTaxPerc <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "SGST @ " & Trim(Val(vTaxPerc)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "SGST ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                End If
                Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("SGst_Amount").ToString), "########0.00"), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font)
                'End If
            End If
            ...
            If Val(prn_HdDt.Rows(0).Item("IGst_Amount").ToString) <> 0 Then
                ...
                'If is_LastPage = True Then
                If vTaxPerc <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "IGST @ " & Trim(Val(vTaxPerc)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "IGST ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                End If
                Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("IGst_Amount").ToString), "########0.00"), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font)
                'End If
            End If
            ...
            If Val(prn_HdDt.Rows(0).Item("Round_Off").ToString) <> 0 Then
                CurY = CurY + TxtHgt
                'If is_LastPage = True Then
                Common_Procedures.Print_To_PrintDocument(e, "Round Off", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 10, CurY, 1, 0, p1Font)
                Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Round_Off").ToString), "########0.00"), LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font)
                'End If
            End If
            ...
            Common_Procedures.Print_To_PrintDocument(e, Cmp_Name, LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10, CurY, 1, 0, p1Font, Brushes.Red)
            Dim TxtWdt As Single = e.Graphics.MeasureString(Cmp_Name, p1Font).Width

            Common_Procedures.Print_To_PrintDocument(e, "For ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 10 - TxtWdt - 5, CurY, 1, 0, p1Font, cM_br)
    ...
    Private Sub QuantityDetails()
        ...
        If Len(cbo_OrderCode.Text) = 0 Then
        ...
        Else
            ...
            t1.Text = Common_Procedures.get_FieldValue(con, "Sales_Delivery_Details", "sum(Quantity)", "OrderCode_forSelection = '" & cbo_OrderCode.Text & "' and Sales_Delivery_Code Like 'EMDEL%' and Sales_Delivery_Date <= '" & Format(dtp_CutOffDate.Value, "dd-MMM-yyyy") & "'" &
                                                          " and Delivery_Purpose = 'Good'", Common_Procedures.CompIdNo)


            s1.Text = Common_Procedures.get_FieldValue(con, "Sales_Details", "sum(Quantity)", "OrderCode_forSelection = '" & cbo_OrderCode.Text & "' and Sales_Code Like 'GINVE%'  " &
            ...
            For I As Int16 = 0 To dgv_Details.Rows.Count - 1
                If Val(txt_SlNo.Text) <> Val(dgv_Details.Rows(I).Cells(0).Value) Then
        ...
        'If Val(txt_Quantity.Text) = 0 Or New_Entry = False Then
        txt_Quantity.Text = b2.Text
        'End If

        For i As Int16 = 0 To dgv_Details.Rows.Count - 1
            If dgv_Details.Rows(i).Cells(21).Value = cbo_OrderCode.Text And dgv_Details.Rows(i).Cells(24).Value = cbo_JobNumber.Text And UCase(dgv_Details.Rows(i).Cells(3).Value) = "ALL" And Val(dgv_Details.Rows(i).Cells(0).Value) <> Val(txt_SlNo.Text) Then
        ...
        If cbo_DCNo.Text = "ALL" Then
            ...
            For i As Int16 = 0 To dgv_Details.Rows.Count - 1
                If dgv_Details.Rows(i).Cells(21).Value = cbo_OrderCode.Text And dgv_Details.Rows(i).Cells(24).Value = cbo_JobNumber.Text And Val(dgv_Details.Rows(i).Cells(0).Value) <> Val(txt_SlNo.Text) Then
    ...
    Private Sub cbo_JobNumber_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_JobNumber.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_JobNumber, txt_SlNo, cbo_DCNo, "OrderJobNo_Head", "OrderJobNo_Name", " OrderNo_Name = '" & cbo_OrderCode.Text & "'", "")
    ...
    Private Sub cbo_JobNumber_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_JobNumber.KeyPress
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_JobNumber, cbo_DCNo, "OrderJobNo_Head", "OrderJobNo_Name", " OrderNo_Name = '" & cbo_OrderCode.Text & "'", "")
    ...
    Private Sub cbo_DCNo_GotFocus(sender As Object, e As EventArgs) Handles cbo_DCNo.GotFocus
        ...
        Try
            ...
            CMD.CommandText = " INSERT INTO Combo_Pop_Temp SELECT convert(varchar,Sales_Delivery_No)+'('+CONVERT(varchar,sales_delivery_date,5)+')' FROM Sales_Delivery_Details  " &
                          " WHERE  not Sales_Delivery_No + '('+ convert(varchar,sales_delivery_date,5)+')' + Job_No + OrderCode_forSelection in (Select Sales_DC_Code + Job_NO + uid from Invoice_DC_Details) " &
                          ...
                          " And Job_No = '" & cbo_JobNumber.Text & "' AND OrderCode_forSelection = '" & cbo_OrderCode.Text & "' and Sales_Delivery_Date <= '" & Format(dtp_CutOffDate.Value, "dd/MMM/yyyy") & "'"
            CMD.ExecuteNonQuery()
    ...
    Private Sub cbo_DCNo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_DCNo.SelectedIndexChanged
    ...
    Private Sub cbo_DCNo_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_DCNo.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_DCNo, cbo_JobNumber, txt_Noof_Stitches, "Combo_Pop_Temp", "LOV", "", "")
    ...
    Private Sub Button1_Click_2(sender As Object, e As EventArgs) Handles btn_Quantity.Click
        ...
        If Trim(UCase(cbo_OrderCode.Text)) <> "" Then
            ...
            da = New SqlClient.SqlDataAdapter("select sum(Quantity) from Sales_Delivery_Details where Job_No = '" & cbo_JobNumber.Text & "' and convert(varchar,Sales_Delivery_No) + '('+ convert(varchar,Sales_Delivery_Date,5) +')' = '" & cbo_DCNo.Text & "' and Delivery_Purpose = 'Good'", con)
            dt = New DataTable
            da.Fill(dt)
