Public Class Payroll_Salary_Entry_Details
    ...
    Private Sub get_PayRoll_Salary_Details()
        ...
        'Dim ESI_MAX_SHFT_WAGES As Single = 0
        'Dim EPF_MAX_BASICPAY As Single = 0
        'Dim EPF_MAX_VALUE As Single = 0
        ...
        Dim vPFSTS As Integer = 0
        Dim vESISTS As Integer = 0
        ...
        With dgv_Details
            ...
            If dt1.Rows.Count > 0 Then
                ...
                For i = 0 To dt1.Rows.Count - 1
                    ...
                    Sal_advance = 0
                    cmd.CommandText = "Select sum(a.Amount) as Sal_Advance from PayRoll_Employee_Payment_Head a Where  a.Employee_IdNo = " & Str(Val(vEmp_IdNo)) & " and a.Employee_Payment_Date between @PreviousAdvanceDate and @AdvanceUpToDate and a.Advance_Salary <> 'SALARY'"
                    ...
                    Else
                        ...
                        If Val(dt1.Rows(i).Item("Week_Attendance_Ot").ToString) = 1 Then
                            Tot_OTMins = OT_Mins + (Shft_Mins * vNoOf_Att_Dys_In_WkOff)
                    ...
                    vESISTS = 0
                    If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1066" Then '---- SOUTHERN COTSPINNERS COIMBATORE LTD (COIMBATORE)  --SPINNING MILL
                        vPFSTS = Val(dt1.Rows(i).Item("Pf_Salary").ToString)
                        vESISTS = Val(dt1.Rows(i).Item("Esi_Salary").ToString)

                    Else
                        vPFSTS = Val(dt1.Rows(i).Item("Pf_Status").ToString)
                        vESISTS = Val(dt1.Rows(i).Item("Esi_Status").ToString)
                    ...
                    If vPFSTS = 1 Then
                        ...
                        If Val(.Rows(n).Cells(25).Value) >= Val(EPF_MAX_BASICPAY) And Val(EPF_MAX_BASICPAY) > 0 Then
                        ...
                        Else
                            ...
                            Else
                                .Rows(n).Cells(47).Value = Format(Math.Ceiling(Val(.Rows(n).Cells(25).Value) * 12 / 100), "#########0.00")
                        ...
                        Else
                            .Rows(n).Cells(48).Value = Format(Math.Round(Val(.Rows(n).Cells(25).Value) * 8.33 / 100), "#########0.00")
                        End If
                        ...
                        'If Val(Val(.Rows(n).Cells(47).Value)) <> 0 Then
                        '    .Rows(n).Cells(49).Value = Format(Val(.Rows(n).Cells(47).Value) - Val(.Rows(n).Cells(48).Value), "#######0.00")
                        'Else
                        '    .Rows(n).Cells(49).Value = Format(Val(.Rows(n).Cells(30).Value) * 3.67 / 100, "#######0.00")
                        'End If
                    ...
                    If Val(dt1.Rows(i).Item("Esi_Status").ToString) = 1 Then
                    ...
                    If Val(dt1.Rows(i).Item("Pf_Status").ToString) = 1 Then
                        ...
                        Else
                            ...
                            Else
                                .Rows(n).Cells(72).Value = Format(Math.Ceiling(Val(.Rows(n).Cells(25).Value) * 12 / 100), "#########0.00")
                        ...
                        Else
                            .Rows(n).Cells(73).Value = Format(Math.Ceiling(Val(.Rows(n).Cells(25).Value) * 8.33 / 100), "#########0.00")
                        End If
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        cmd.CommandText = "select * from PayRoll_Salary_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Salary_Payment_Type_IdNo = " & Str(Val(SalPymtTyp_IdNo)) & " and Salary_Code <> '" & Trim(NewCode) & "' and ( (@SalaryFromDate Between From_Date and To_Date) or (@SalaryToDate Between From_Date and To_Date) )"
        Da = New SqlClient.SqlDataAdapter(cmd)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
        ...
        Try
            ...
            If New_Entry = True Then
                cmd.CommandText = "Insert into PayRoll_Salary_Head (     Salary_Code        ,               Company_IdNo       ,           Salary_No           ,                               for_OrderBy                              ,   Salary_Date ,       Salary_Payment_Type_IdNo   ,             Category_IdNo     ,         Month_IdNo   ,  From_Date,  To_Date,  Advance_UptoDate,                 Total_Days          ,                  Festival_Days           ) " & _
                                    "          Values              ( '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ",  @SalaryDate  , " & Str(Val(SalPymtTyp_IdNo)) & ", " & Str(Val(vCatgry_IdNo)) & ", " & Val(Mth_IDNo) & ", @FromDate , @ToDate , @AdvanceUpToDate , " & Str(Val(txt_TotalDays.Text)) & ",  " & Str(Val(txt_FestivalDays.Text)) & " ) "
                cmd.ExecuteNonQuery()
    ...
    '    Dim RowHeight1 As Integer = dgv_Details.Rows(e.RowIndex).Height
    '    Dim CellRectangle1 As Rectangle = dgv_Details.GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, False)

    '    CellRectangle1.X += dgv_Details.Left
    '    CellRectangle1.Y += dgv_Details.Top + RowHeight1
    'End Sub
    ...
    Private Sub dgv_Details_CellLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellLeave
        Try
            ...
            '            If e.ColumnIndex = 50 Or e.ColumnIndex = 52 Then
            '                If Val(.CurrentRow.Cells(50).Value) <> 0 Or Val(.CurrentRow.Cells(52).Value) <> 0 Then

            '                    .CurrentRow.Cells(54).Value = Format((((Val(.CurrentRow.Cells().Value) - Val(.CurrentRow.Cells(50).Value))) - Val(.CurrentRow.Cells(52).Value)) + Val(.CurrentRow.Cells(53).Value), "##########0.00")
            ...
            With dgv_Details
                ...
                If .Visible Then
                    ...
                    If .Rows.Count > 0 Then
                        ...
                        If e.ColumnIndex = 3 Or e.ColumnIndex = 28 Or e.ColumnIndex = 42 Or e.ColumnIndex = 53 Or e.ColumnIndex = 54 Then
                        ...
                        If e.ColumnIndex = 55 Or e.ColumnIndex = 56 Or e.ColumnIndex = 57 Or e.ColumnIndex = 59 Or e.ColumnIndex = 60 Then
                            ...
                            Else
                                If Val(.CurrentRow.Cells(6).Value) <> 0 Then
                                    ...
                                    'esi
                                    If Val(.CurrentRow.Cells(46).Value) <> 0 Then

                                        .CurrentRow.Cells(46).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 1.75 / 100), "##########0.00")
                                    End If
                                    If Val(.CurrentRow.Cells(71).Value) <> 0 Then
                                        .CurrentRow.Cells(71).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 1.75 / 100), "##########0.00")
                                    End If
                                    ...
                                    If Val(.CurrentRow.Cells(25).Value) >= Val(EPF_MAX_BASICPAY) And Val(EPF_MAX_BASICPAY) > 0 Then
                                        ...
                                        .CurrentRow.Cells(49).Value = Format(Math.Ceiling(Val(EPF_MAX_BASICPAY) * 3.67 / 100), "#########0.00")
                                    ...
                                    Else
                                        If Val(.CurrentRow.Cells(47).Value) <> 0 Then
                                            .CurrentRow.Cells(47).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 12 / 100), "#########0.00")
                                        End If



                                        If Val(.CurrentRow.Cells(72).Value) <> 0 Then
                                            .CurrentRow.Cells(72).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 12 / 100), "#########0.00")
                                        End If

                                        'pf
                                        If Val(.CurrentRow.Cells(48).Value) <> 0 Then
                                            .CurrentRow.Cells(48).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 8.33 / 100), "#########0.00")
                                        End If
                                        If Val(.CurrentRow.Cells(73).Value) <> 0 Then
                                            .CurrentRow.Cells(73).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 8.33 / 100), "#########0.00")
                                        End If

                                        'ps
                                        If Val(.CurrentRow.Cells(49).Value) <> 0 Then
                                            .CurrentRow.Cells(49).Value = Format(Math.Ceiling(Val(.CurrentRow.Cells(25).Value) * 3.67 / 100), "#########0.00")
                                        End If
                                ...
                                Else
                                    .CurrentRow.Cells(61).Value = 0 ' Net pay
                                    '.CurrentRow.Cells(29).Value = ""
                                    '.CurrentRow.Cells(30).Value = ""
                                    '.CurrentRow.Cells(31).Value = ""
                                    '.CurrentRow.Cells(32).Value = ""
                                    '.CurrentRow.Cells(33).Value = ""
                                    '.CurrentRow.Cells(34).Value = ""
                                    '.CurrentRow.Cells(35).Value = ""
                                    '.CurrentRow.Cells(36).Value = ""
                                    '.CurrentRow.Cells(37).Value = ""
                                    '.CurrentRow.Cells(38).Value = ""
                                    '.CurrentRow.Cells(39).Value = ""
                                    '.CurrentRow.Cells(40).Value = ""
                                    '.CurrentRow.Cells(41).Value = ""
                                    '.CurrentRow.Cells(42).Value = ""
                                    '.CurrentRow.Cells(43).Value = ""
                                    '.CurrentRow.Cells(44).Value = ""
                                    '.CurrentRow.Cells(45).Value = ""
                                    '.CurrentRow.Cells(46).Value = ""
                                    '.CurrentRow.Cells(47).Value = ""
                                    '.CurrentRow.Cells(48).Value = ""
                                    '.CurrentRow.Cells(49).Value = ""
                                    '.CurrentRow.Cells(50).Value = ""
                                    '.CurrentRow.Cells(51).Value = ""
                                    '.CurrentRow.Cells(52).Value = ""
                                    '.CurrentRow.Cells(53).Value = ""
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.* from PayRoll_Salary_Head a INNER JOIN Company_Head b ON a.Company_IdNo = b.Company_IdNo  Where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Salary_Code ='" & Trim(NewCode) & "' Order by a.for_orderby, a.Salary_No", con)
    ...
    Private Sub Printing_Format2(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClArr(1) = 200 : ClArr(2) = 100 : ClArr(3) = 110
        'ClArr(2) = PageWidth - (LMargin + ClArr(1))
                ...
                If prn_DetDt.Rows.Count > 0 Then
                    ...
                    Do While prn_DetIndx <= prn_DetDt.Rows.Count - 1
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Total_Days").ToString) > 0 Then
                            Common_Procedures.Print_To_PrintDocument(e, "1 Present", LMargin + 10, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Total_Days").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("OT_Hours").ToString) > 0 Then
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("OT_Hours").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If
                        CurY = CurY + TxtHgt
                        e.Graphics.DrawLine(Pens.Black, LMargin, CurY, PageWidth, CurY)
                        LnAr(6) = CurY
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, "Amount", LMargin + ClArr(1) + ClArr(2) + ClArr(3), CurY, 0, 0, p2font)
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, "Basic Pay", LMargin + 10, CurY, 0, 0, p1Font)
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Earning").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        ''Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Total_Advance").ToString, PageWidth - 10, CurY, 1, 0, pFont)
                        'CurY = CurY + TxtHgt - 8

                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Mess").ToString) > 0 Then
                            Common_Procedures.Print_To_PrintDocument(e, "Mess", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("mess").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                        'Common_Procedures.Print_To_PrintDocument(e, "Miniumum Wages Earned in the Week", LMargin + 10, CurY, 0, 0, p1Font)

                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("H_R_A").ToString) > 0 Then
                            CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "HRA ", LMargin + 10, CurY, 0, 0, p1Font)
                            ...
                            'Common_Procedures.Print_To_PrintDocument(e, "@ 12 %", LMargin + ClArr(1) + 170, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("H_R_A").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Medical").ToString) > 0 Then
                            'CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "Medical", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Medical").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Conveyance").ToString) > 0 Then
                            CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "Conveyance ", LMargin + 10, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Conveyance").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("ESI").ToString) > 0 Then
                            'CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "ESI 1.75%", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("ESI").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                       

                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("washing").ToString) > 0 Then
                            CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "Washing ", LMargin + 10, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("washing").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If

                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("P_F").ToString) > 0 Then
                            'CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "P F 12%", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("P_F").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Entertainment").ToString) > 0 Then
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Entertainment").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p1Font)
                        End If
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Store").ToString) > 0 Then
                            CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "Store", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Store").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                        ...
                        If Val(prn_DetDt.Rows(prn_DetIndx).Item("Late_Hours_Salary").ToString) > 0 Then
                            CurY = CurY + TxtHgt
                            Common_Procedures.Print_To_PrintDocument(e, "Late Hours Salary", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p1Font)
                            Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Late_Hours_Salary").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p1Font)
                        End If
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Total_Addition").ToString, LMargin + ClArr(1) + 50, CurY, 1, 0, p2font) 'LMargin + ClArr(1) - 15, CurY, 0, 0, p2font)
                        Common_Procedures.Print_To_PrintDocument(e, "Total Deductions", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p2font)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Total_Deduction").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p2font)
                        CurY = CurY + TxtHgt

                        Common_Procedures.Print_To_PrintDocument(e, "Net Amount", LMargin + ClArr(1) + ClArr(2) - 35, CurY, 0, 0, p2font)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Net_Salary").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + 65, CurY, 1, 0, p2font)
                        e.Graphics.DrawLine(Pens.Black, LMargin, CurY, PageWidth, CurY)
                        LnAr(8) = CurY
    ...
    Private Sub Printing_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 108.5, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_DetDt.Rows(prn_DetIndx).Item("Card_No").ToString, LMargin + 115, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, "je;ij ngah;", LMargin + ClAr(1) + 10, CurY, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, ": " & prn_DetDt.Rows(prn_DetIndx).Item("Father_Husband").ToString, LMargin + ClAr(1) + 110, CurY, 0, 0, pFont)
            ...
            'Common_Procedures.Print_To_PrintDocument(e, "/ ", LMargin + 100, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, "DOB :" & prn_DetDt.Rows(0).Item("Date_Birth").ToString, LMargin + 110, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, "DOB ", LMargin + 10, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 108.5, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_DetDt.Rows(0).Item("Date_Birth").ToString, LMargin + 115, CurY, 0, 0, pFont)

            ' uan no and pan no need to add
            

            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, "PF NO  ", LMargin + 10, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 108.5, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_DetDt.Rows(0).Item("Pf_No").ToString, LMargin + 115, CurY, 0, 0, pFont)
            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, "ESI NO  ", LMargin + 10, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 108.5, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_DetDt.Rows(prn_DetIndx).Item("Esi_No").ToString, LMargin + 115, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, "gp.vg;.vz;", LMargin + 220, CurY, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, ",jug;gbfs;", LMargin + ClAr(1) + 10, CurY, 0, 0, p1Font)
            'Common_Procedures.Print_To_PrintDocument(e, "Allowances", LMargin + ClAr(1) + 130, CurY, 0, 0, pFont)
            'Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + ClAr(1) + 200, CurY, 0, 0, pFont)
            ...
            e.Graphics.DrawLine(Pens.Black, LMargin, CurY, PageWidth - ClAr(1) - ClAr(2) - ClAr(3) - 56, CurY)
            LnAr(4) = CurY
    ...
    Private Sub Printing_Format2_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean)
        ...
        e.Graphics.DrawLine(Pens.Black, LMargin + 480, LnAr(5), LMargin + 480, LnAr(6))
