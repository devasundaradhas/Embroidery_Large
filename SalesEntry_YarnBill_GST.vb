Public Class SalesEntry_YarnBill_GST
            ...
            If dt1.Rows.Count > 0 Then
                ...
                If dt2.Rows.Count > 0 Then
                    ...
                    For i = 0 To dt2.Rows.Count - 1
                        ...
                        dgv_Details.Rows(n).Cells(2).Value = IIf(Trim(dt2.Rows(i).Item("Item_Description").ToString) <> "", dt2.Rows(i).Item("Item_Description").ToString, dt2.Rows(i).Item("Serial_No").ToString)
                        ...
                        dgv_Details.Rows(n).Cells(8).Value = dt2.Rows(i).Item("Sales_Delivery_Code").ToString
                        dgv_Details.Rows(n).Cells(9).Value = dt2.Rows(i).Item("Sales_Delivery_Detail_SlNo").ToString
                        dgv_Details.Rows(n).Cells(10).Value = dt2.Rows(i).Item("Sales_Detail_SlNo").ToString
    ...
    Private Sub SalesEntry_YarnBill_GST_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1141" Then  '---  SRI SABARI TRADERS (TIRUPUR) 
            ...
            dgv_Details_Total.Columns(5).Visible = False
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        If dgv_Details_Total.RowCount > 0 Then
            ...
            vTot_Wgt = Val(dgv_Details_Total.Rows(0).Cells(5).Value)
        End If
        ...
        Try
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Val(dgv_Details.Rows(i).Cells(4).Value) <> 0 Then
                        ...
                        cmd.CommandText = "Insert into Sales_Details ( Sales_Code,            Company_IdNo          ,              Sales_No             ,                                              for_OrderBy                   , Sales_Date,          Ledger_IdNo    ,            Sl_No     ,          Item_IdNo      ,     ItemGroup_IdNo         ,           Item_Description             ,          Unit_IdNo      ,                       Noof_Items         ,                      Weight              ,                      Rate                ,                      Amount               ,                      Total_Amount         ,               Entry_Type         ,                    Sales_Delivery_Code  ,               Sales_dELIVERY_Detail_SlNo  ,      Cash_Discount_Perc_For_All_Item     ,       Cash_Discount_Amount_For_All_Item   ,              Assessable_Value             ,                    HSN_Code             ,                      Tax_Perc              ) " & _
                                                " Values ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_InvoiceNo.Text))) & ", @SalesDate, " & Str(Val(led_id)) & ", " & Str(Val(Sno)) & ", " & Str(Val(itm_id)) & ", " & Str(Val(itm_GrpId)) & ",'" & Trim(.Rows(i).Cells(2).Value) & "' , " & Str(Val(unt_id)) & ", " & Str(Val(.Rows(i).Cells(4).Value)) & ", " & Str(Val(.Rows(i).Cells(5).Value)) & ", " & Str(Val(.Rows(i).Cells(6).Value)) & ", " & Str(Val(.Rows(i).Cells(7).Value)) & " , " & Str(Val(.Rows(i).Cells(7).Value)) & " , '" & Trim(cbo_EntType.Text) & "' , '" & Trim(.Rows(i).Cells(8).Value) & "' ,  " & Str(Val(.Rows(i).Cells(9).Value)) & "," & Str(Val(.Rows(i).Cells(11).Value)) & ", " & Str(Val(.Rows(i).Cells(12).Value)) & ", " & Str(Val(.Rows(i).Cells(13).Value)) & ", '" & Trim(.Rows(i).Cells(14).Value) & "', " & Str(Val(.Rows(i).Cells(15).Value)) & " ) "
                        cmd.ExecuteNonQuery()

                        If Trim(UCase(cbo_EntType.Text)) = "DELIVERY" Then

                            cmd.CommandText = "Update Sales_Delivery_Details Set Receipt_Quantity = Receipt_Quantity + " & Str(Val(.Rows(i).Cells(5).Value)) & " where Sales_Delivery_Code = '" & Trim(.Rows(i).Cells(8).Value) & "' and Sales_Delivery_Detail_SlNo = " & Str(Val(.Rows(i).Cells(9).Value)) & " and Ledger_IdNo = " & Str(Val(led_id))
            ...
            If Common_Procedures.Voucher_Updation(con, Trim(VouType), Val(lbl_Company.Tag), Trim(Pk_Condition) & Trim(NewCode), Trim(lbl_InvoiceNo.Text), dtp_Date.Value.Date, "Bill No . : " & Trim(lbl_InvoiceNo.Text), vVouPos_IdNos, vVouPos_Amts, vVouPos_ErrMsg, tr) = False Then
    ...
    Private Sub btn_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Add.Click
        ...
        If Trim(cbo_Unit.Text) <> "" Then
            ...
            If Val(Sz_id) = 0 Then
                MessageBox.Show("Invalid Size", "DOES NOT ADD...", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk)
            ...
            With dgv_Details
                ...
                For i = 0 To .Rows.Count - 1
                    If Val(dgv_Details.Rows(i).Cells(0).Value) = Val(txt_SlNo.Text) Then
                        ...
                        lbl_Grid_HsnCode.Text = Trim(dgv_Details.CurrentRow.Cells(14).Value)
                ...
                If .Visible Then
                    If Trim(UCase(cbo_EntType.Text)) = "DELIVERY" Then
                        If .CurrentCell.ColumnIndex = 4 Or .CurrentCell.ColumnIndex = 5 Or .CurrentCell.ColumnIndex = 6 Then
                            If .Rows(.CurrentCell.RowIndex).Cells(4).Value <> 0 Then
                                .Rows(.CurrentCell.RowIndex).Cells(7).Value = Format(Val(.Rows(.CurrentCell.RowIndex).Cells(4).Value) * Val(.Rows(.CurrentCell.RowIndex).Cells(6).Value), "#########0.00")
                            Else
    ...
    Private Sub GrossAmount_Calculation()
        ...
        With dgv_Details
            ...
            For I = 0 To .RowCount - 1
                ...
                If Trim(.Rows(I).Cells(1).Value) <> "" Or Val(.Rows(I).Cells(4).Value) <> 0 Or Val(.Rows(I).Cells(5).Value) <> 0 Then
                    TotWgt = TotWgt + Val(dgv_Details.Rows(I).Cells(4).Value)
        ...
        With dgv_Details_Total
            ...
            .Rows(0).Cells(4).Value = Val(TotWgt)
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1002" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1150" Then
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Trim(.Rows(i).Cells(1).Value) <> "" Then
                        ...
                        If ItmIdNo <> 0 Then
                            ...
                            .Rows(i).Cells(12).Value = Format(Val(.Rows(i).Cells(7).Value) * Val(.Rows(i).Cells(11).Value) / 100, "#########0.00")
                            .Rows(i).Cells(13).Value = Format(Val(.Rows(i).Cells(7).Value) - Val(.Rows(i).Cells(12).Value), "#########0.00")
    ...
    Private Sub TotalAmount_Calculation()
        ...
        For i = 0 To dgv_Details.RowCount - 1
            ...
            If Trim(dgv_Details.Rows(i).Cells(1).Value) <> "" Or Val(dgv_Details.Rows(i).Cells(4).Value) <> 0 Or Val(dgv_Details.Rows(i).Cells(5).Value) <> 0 Then
                TotWgt = TotWgt + Val(dgv_Details.Rows(i).Cells(4).Value)
                TotQty = TotQty + Val(dgv_Details.Rows(i).Cells(5).Value)
                ...
                Try
                    ...
                    If prn_DetDt.Rows.Count > 0 Then
                        ...
                        Do While DetIndx <= prn_DetDt.Rows.Count - 1
                            ...
                            If Trim(prn_DetDt.Rows(DetIndx).Item("Item_Description").ToString) <> "" Or Trim(prn_DetDt.Rows(DetIndx).Item("Serial_No").ToString) <> "" Then
                                ItmNm1 = Trim(ItmNm1) & "  -  " & IIf(Trim(prn_DetDt.Rows(DetIndx).Item("Item_Description").ToString) <> "", Trim(prn_DetDt.Rows(DetIndx).Item("Item_Description").ToString), Trim(prn_DetDt.Rows(DetIndx).Item("Serial_No").ToString))
                            End If
...
LOOP1:
    ...
    Private Sub Printing_GST_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        Try
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1002" Then '---- Peacock Traders(Tirupur)
                vLedContNm = Common_Procedures.get_FieldValue(con, "Company_Head", "Company_ContactPerson", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & ")")
                If InStr(1, Trim(vLedContNm), "YUVA") > 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "KVI-" & prn_HdDt.Rows(0).Item("Sales_No").ToString, LMargin + W1 + 30, CurY1 - 3, 0, 0, p1Font)
                Else
                ...
                Try
                    ...
                    If prn_DetDt.Rows.Count > 0 Then
                        ...
                        Do While DetIndx <= prn_DetDt.Rows.Count - 1
                            ...
                            Dim QtyTxt As String = "", QtyCap As String = ""
                            Dim vUOM As String = ""

                            QtyTxt = ""
                            If Val(prn_DetDt.Rows(DetIndx).Item("Weight").ToString) <> 0 Then
                                QtyTxt = Trim(Format(Val(prn_DetDt.Rows(DetIndx).Item("Weight").ToString), "#########0.000"))
                            Else
                                QtyTxt = Trim(Val(prn_DetDt.Rows(DetIndx).Item("Noof_Items").ToString))
                            End If

                            vUOM = Common_Procedures.Unit_IdNoToName(con, Val(prn_DetDt.Rows(DetIndx).Item("Unit_IdNO").ToString))

                            QtyCap = Space(8 - Len(Trim(Microsoft.VisualBasic.Left(QtyTxt, 8)))) & Trim(QtyTxt) & " " & Trim(Microsoft.VisualBasic.Left(vUOM, 5)) & Space(5 - Len(Trim(Microsoft.VisualBasic.Left(vUOM, 5))))
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, Trim(Val(DetSNo)), LMargin + 60, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, Trim(ItmNm1), LMargin + 110, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, "HSN : " & prn_DetDt.Rows(DetIndx).Item("Hsn_Code").ToString, LMargin + 280, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, QtyCap, LMargin + 545, CurY, 1, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_DetDt.Rows(DetIndx).Item("Rate").ToString), "########0.00"), LMargin + 620, CurY, 1, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_DetDt.Rows(DetIndx).Item("Amount").ToString), "########0.00"), 735, CurY, 1, 0, pFont)

                            NoofDets = NoofDets + 1

                            If Trim(ItmNm2) <> "" Then
                                CurY = CurY + TxtHgt - 5
                                Common_Procedures.Print_To_PrintDocument(e, Trim(ItmNm2), LMargin + 120, CurY, 0, 0, pFont)
                                NoofDets = NoofDets + 1
                            End If
    ...
    Private Sub Printing_GST_Format3_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        If PageNo > 1 Then
            ...
            da2.Fill(dt2)
            If dt2.Rows.Count > NoofItems_PerPage Then
                Common_Procedures.Print_To_PrintDocument(e, "Page : " & Trim(Val(PageNo)), 735, 20, 1, 0, pFont)
            End If
            dt2.Clear()
        End If
        ...
        If PageNo <= 1 Then
            Common_Procedures.Print_To_PrintDocument(e, Trim(prn_OriDupTri), LMargin + 735, TMargin + 20, 1, 0, pFont)
        End If

        Cmp_Email = ""
        Cmp_StateCap = "" : Cmp_StateNm = "" : Cmp_StateCode = ""
        Cmp_PanNo = "" : Cmp_PanCap = ""
        Cmp_GSTIN_Cap = "" : Cmp_GSTIN_No = ""
        ...
        p1Font = New Font("Calibri", 11, FontStyle.Bold)
        Common_Procedures.Print_To_PrintDocument(e, Cmp_GSTIN_No & "  /  " & Cmp_Email, LMargin + 220, TMargin + 167, 0, 0, p1Font)


        Common_Procedures.Print_To_PrintDocument(e, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", LMargin + 185, TMargin + 185, 0, 0, pFont)
        ...
        CurY = CurY + TxtHgt - 15
        p1Font = New Font("Calibri", 16, FontStyle.Bold)
        Common_Procedures.Print_To_PrintDocument(e, vHeading, LMargin + 400, TMargin + 220, 2, 340, p1Font)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, "Bill No", LMargin + 420, TMargin + 270, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 420 + W1, TMargin + 270, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, "G-" & Trim(Format(Val(Inv_No), "######000")) & Trim(InvSubNo), LMargin + 420 + W1 + 30, TMargin + 270, 0, 0, p1Font)

            p1Font = New Font("Calibri", 12, FontStyle.Bold)
            Common_Procedures.Print_To_PrintDocument(e, "Date", LMargin + 420, TMargin + 305, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + 420 + W1, TMargin + 305, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Sales_Date").ToString), "dd-MM-yyyy").ToString, LMargin + 420 + W1 + 30, TMargin + 305, 0, 0, p1Font)
            ...
            CurY = TMargin + 220
            Common_Procedures.Print_To_PrintDocument(e, "M/s. " & prn_HdDt.Rows(0).Item("Ledger_Name").ToString, LMargin + 60, CurY, 0, 0, p1Font)

            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_HdDt.Rows(0).Item("Ledger_Address1").ToString, LMargin + 60, CurY, 0, 0, pFont)

            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_HdDt.Rows(0).Item("Ledger_Address2").ToString, LMargin + 60, CurY, 0, 0, pFont)

            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_HdDt.Rows(0).Item("Ledger_Address3").ToString, LMargin + 60, CurY, 0, 0, pFont)

            CurY = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, " " & prn_HdDt.Rows(0).Item("Ledger_Address4").ToString, LMargin + 60, CurY, 0, 0, pFont)

            If Trim(prn_HdDt.Rows(0).Item("Ledger_GSTinNo").ToString) <> "" Then
                CurY = CurY + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, " GSTIN  :  " & prn_HdDt.Rows(0).Item("Ledger_GSTinNo").ToString, LMargin + 60, CurY, 0, 0, pFont)
            End If

            'Common_Procedures.Print_To_PrintDocument(e, "HSN CODE", LMargin + 320, TMargin + 380, 0, 0, pFont)
    ...
    Private Sub Printing_GST_Format3_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal Cmp_Name As String, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean)
        ...
        Dim Jurs As String = ""
        Dim vNoofHsnCodes As Integer = 0
        Dim vDcNo1 As String = "", vDcNo2 As String = ""
        Dim vDcNo_CurY As Single = 0
        ...
        Try
            ...
            If is_LastPage = True Then
                ...
                If Val(prn_HdDt.Rows(0).Item("CashDiscount_Amount").ToString) <> 0 Or Val(prn_HdDt.Rows(0).Item("Freight_Amount").ToString) <> 0 Or Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString) <> 0 Then
                    e.Graphics.DrawLine(Pens.Black, LMargin + 559, CurY, 748, CurY)
                    'e.Graphics.DrawLine(Pens.Black, LMargin + 641, CurY, 748, CurY)
                    CurY = CurY + TxtHgt
                    Common_Procedures.Print_To_PrintDocument(e, "TOTAL", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Gross_Amount").ToString), "########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                End If

                If Val(prn_HdDt.Rows(0).Item("CashDiscount_Amount").ToString) <> 0 Then
                    CurY = CurY + TxtHgt
                    Common_Procedures.Print_To_PrintDocument(e, "Cash Discount ", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "@ " & Trim(Val(prn_HdDt.Rows(0).Item("CashDiscount_Perc").ToString)) & "%", LMargin + 620, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("CashDiscount_Amount").ToString), "########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                End If

                If Val(prn_HdDt.Rows(0).Item("Freight_Amount").ToString) <> 0 Then
                    CurY = CurY + TxtHgt
                    Common_Procedures.Print_To_PrintDocument(e, "Freight Charge", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Freight_Amount").ToString), "########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                End If

                If Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString) <> 0 Then
                    CurY = CurY + TxtHgt
                    If Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString) > 0 Then
                        Common_Procedures.Print_To_PrintDocument(e, "Add Amount", LMargin + 547, CurY, 1, 0, pFont)
                    Else
                        Common_Procedures.Print_To_PrintDocument(e, "Less Amount", LMargin + 547, CurY, 1, 0, pFont)
                    End If
                    Common_Procedures.Print_To_PrintDocument(e, Format(Math.Abs(Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString)), "########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                ...
                CurY = CurY + TxtHgt
                p1Font = New Font("Calibri", 12, FontStyle.Bold)
                Common_Procedures.Print_To_PrintDocument(e, "TOTAL", LMargin + 547, CurY, 1, 0, p1Font)
                Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("Assessable_Value").ToString), "##########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                ...
                CurY = CurY + TxtHgt
                If Val(prn_HdDt.Rows(0).Item("CGst_Amount").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "Add : CGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "@ " & Format(Val(vTaxPerc), "##########0.0") & " %", LMargin + 620, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("CGst_Amount").ToString), "##########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "Add : CGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "0.00", LMargin + 735, CurY, 1, 0, pFont)
                End If

                CurY = CurY + TxtHgt
                If Val(prn_HdDt.Rows(0).Item("SGst_Amount").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "Add : SGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "@ " & Format(Val(vTaxPerc), "##########0.0") & " %", LMargin + 620, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("SGst_Amount").ToString), "##########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "Add : SGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "0.00", LMargin + 735, CurY, 1, 0, pFont)
                End If
                ...
                If Val(prn_HdDt.Rows(0).Item("IGst_Amount").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "Add : IGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "@ " & Format(Val(vTaxPerc), "##########0.0") & " %", LMargin + 620, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("IGst_Amount").ToString), "##########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "Add : IGST", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "0.00", LMargin + 735, CurY, 1, 0, pFont)
                End If
                ...
                CurY = CurY + TxtHgt
                If Val(prn_HdDt.Rows(0).Item("Round_Off").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, "ROUND OFF", LMargin + 547, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("Round_Off").ToString), "##########0.00"), LMargin + 735, CurY, 1, 0, pFont)
                End If

                p1Font = New Font("Calibri", 12, FontStyle.Bold)
                Common_Procedures.Print_To_PrintDocument(e, Common_Procedures.Currency_Format(Val(prn_HdDt.Rows(0).Item("Net_Amount").ToString)), LMargin + 735, TMargin + 870, 1, 0, p1Font)


                BmsInWrds = Common_Procedures.Rupees_Converstion(Val(prn_HdDt.Rows(0).Item("Net_Amount").ToString))
                p1Font = New Font("Calibri", 12, FontStyle.Bold)
                Common_Procedures.Print_To_PrintDocument(e, " Rupees  :  " & StrConv(BmsInWrds, VbStrConv.ProperCase), LMargin + 60, TMargin + 930, 0, 0, p1Font)



                If Trim(prn_HdDt.Rows(0).Item("Dc_No").ToString) <> "" Then

                    vDcNo1 = Trim(prn_HdDt.Rows(0).Item("Dc_No").ToString)
                    vDcNo2 = ""
                    If Len(vDcNo1) > 15 Then
                        For I = 15 To 1 Step -1
                            If Mid$(Trim(vDcNo1), I, 1) = " " Then Exit For
                        Next I
                        If I = 0 Then I = 15
                        vDcNo2 = Microsoft.VisualBasic.Right(Trim(vDcNo1), Len(vDcNo1) - I)
                        vDcNo1 = Microsoft.VisualBasic.Left(Trim(vDcNo1), I)
                    End If

                    Common_Procedures.Print_To_PrintDocument(e, vDcNo1, LMargin + 110, vDcNo_CurY, 0, 0, pFont)

                    If Trim(vDcNo2) <> "" Then
                        vDcNo_CurY = vDcNo_CurY + TxtHgt
                        Common_Procedures.Print_To_PrintDocument(e, vDcNo2, LMargin + 130, vDcNo_CurY, 0, 0, pFont)
                    End If
