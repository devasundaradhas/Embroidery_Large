Public Class Purchase_Entry_Simple_Gst
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name as PartyName, c.Ledger_Name as PurchaseAcName from Purchase_Head a LEFT OUTER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo LEFT OUTER JOIN Ledger_Head c ON a.PurchaseAc_IdNo = c.Ledger_IdNo where a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and a.Entry_VAT_GST_Type = 'GST'", con)
                ...
                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Unit_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Unit_Head c on b.unit_idno = c.unit_idno where a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' Order by a.sl_no", con)
                    ...
                    If dt2.Rows.Count > 0 Then
                        ...
                        dgv_Details.Rows(n).Cells(2).Value = dt2.Rows(i).Item("Description").ToString
                        ...
                        dgv_Details.Rows(n).Cells(18).Value = dt2.Rows(i).Item("OrderCode_ForSelection").ToString
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            Else
                ...
                cmd.CommandText = "Update Purchase_Head set Entry_VAT_GST_Type = 'GST' , Purchase_Date = @PurchaseDate, Payment_Method = '" & Trim(cbo_PaymentMethod.Text) & "', Ledger_IdNo = " & Str(Val(led_id)) & ", PurchaseAc_IdNo = " & Str(Val(purcac_id)) & ", Tax_Type = 'VAT', Narration = '" & Trim(txt_Narration.Text) & "',Freight_Amount = " & Str(Val(txt_Freight.Text)) & "  ,  Total_Qty = " & Str(Val(txt_TotalQty.Text)) & ", SubTotal_Amount = " & Str(Val(txt_SubTotal.Text)) & ", Total_DiscountAmount = " & Str(Val(txt_TotalDiscAmount.Text)) & ", Total_TaxAmount = " & Str(Val(lbl_TotalTaxAmount.Text)) & ", Gross_Amount = " & Str(Val(txt_GrossAmount.Text)) & ", CashDiscount_Perc = " & Str(Val(txt_CashDiscPerc.Text)) & ", CashDiscount_Amount = " & Str(Val(txt_CashDiscAmount.Text)) & ", AddLess_Amount = " & Str(Val(txt_AddLessAmount.Text)) & ", Round_Off = " & Str(Val(txt_RoundOff.Text)) & ", Net_Amount = " & Str(Val(txt_NetAmount.Text)) & " ,Bill_No = '" & Trim(txt_BillNo.Text) & "', Electronic_Reference_No = '" & Trim(txt_Electronic_RefNo.Text) & "' ,  Transportation_Mode = '" & Trim(cbo_TransportMode.Text) & "'  ,  Date_Time_Of_Supply = '" & Trim(txt_DateTime_Of_Supply.Text) & "'  , Entry_GST_Tax_Type = '" & Trim(cbo_TaxType.Text) & "',  CGst_Amount = " & Str(Val(lbl_CGstAmount.Text)) & " , SGst_Amount = " & Str(Val(lbl_SGstAmount.Text)) & " , IGst_Amount = " & Str(Val(lbl_IGstAmount.Text)) & ",Place_Of_Supply = '" & Trim(txt_place_Supply.Text) & "',Assessable_Value = " & Str(Val(txt_GrossAmount.Text) - Val(txt_DiscountAmount.Text)) & " Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
                cmd.ExecuteNonQuery()
            ...
            If Common_Procedures.Voucher_Updation(con, "Gst.Purc", Val(lbl_Company.Tag), Trim(Pk_Condition) & Trim(NewCode), Trim(lbl_PurchaseNo.Text), dtp_Date.Value.Date, "Bill No : " & Trim(txt_BillNo.Text), vVouPos_IdNos, vVouPos_Amts, vVouPos_ErrMsg, tr) = False Then
            ...
            ' Bill(Posting)
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), dtp_Date.Text, AcPos_ID, Trim(txt_BillNo.Text), 0, Val(CSng(txt_NetAmount.Text)), "CR", Trim(Pk_Condition) & Trim(NewCode), tr)
        ...
        With dgv_Details
            ...
            For i = 0 To .Rows.Count - 1
                If Val(dgv_Details.Rows(i).Cells(0).Value) = Val(txt_SlNo.Text) Then
                    ...
                    .Rows(i).Cells(6).Value = Format(Val(txt_TaxRate.Text), "########0.000")
                    ...
                    .Rows(i).Cells(18).Value = cbo_OrderNo.Text
            ...
            If MtchSTS = False Then
                ...
                .Rows(n).Cells(6).Value = Format(Val(txt_TaxRate.Text), "########0.000")
                ...
                .Rows(n).Cells(18).Value = cbo_OrderNo.Text
    ...
    Private Sub txt_Rate_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_Rate.KeyUp
        txt_TaxRate.Text = Format(Val(txt_Rate.Text) * ((100 + Val(txt_TaxPerc.Text)) / 100), "##########0.000")
        Call Amount_Calculation(False)
    End Sub
    ...
    Private Sub get_Item_Unit_Rate_TaxPerc()
        ...
        If dt.Rows.Count > 0 Then
            ...
            If Val(txt_Rate.Text) = 0 Then
                If IsDBNull(dt.Rows(0)("Cost_Rate").ToString) = False Then
                    txt_Rate.Text = dt.Rows(0)("Cost_Rate").ToString
                End If
            End If
            If IsDBNull(dt.Rows(0)("GST_Percentage").ToString) = False Then
                txt_TaxRate.Text = dt.Rows(0)("GST_Percentage").ToString
            End If
    ...
    Private Sub cbo_Ledger_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_Ledger.GotFocus
        ...
        If Trim(Common_Procedures.settings.CustomerCode) = "1039" Or Trim(Common_Procedures.settings.CustomerCode) = "1133" Then
            ...
            With dgv_Details
                ...
                For i = 0 To .Rows.Count - 1
                    If Val(dgv_Details.Rows(i).Cells(0).Value) = Val(txt_SlNo.Text) Then
                        ...
                        txt_Rate.Text = Format(Val(.Rows(i).Cells(5).Value), "########0.000")
                        ...
                        lbl_Grid_HsnCode.Text = Trim(.Rows(i).Cells(17).Value)
                        Exit For
    ...
    Private Sub txt_TaxRate_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_TaxRate.KeyUp
        txt_Rate.Text = Format(Val(txt_TaxRate.Text) * (100 / (100 + Val(txt_TaxPerc.Text))), "#########0.000")
        Amount_Calculation(True)
    End Sub
    ...
    Private Sub dgv_Details_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgv_Details.DoubleClick
        ...
        If Trim(dgv_Details.CurrentRow.Cells(1).Value) <> "" Then
            ...
            cbo_OrderNo.Text = dgv_Details.CurrentRow.Cells(18).Value
    ...
    Private Sub cbo_PaymentMethod_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PaymentMethod.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PaymentMethod, dtp_Date, cbo_TaxType, "", "", "", "")
    End Sub
    ...
    Private Sub Amount_Calculation(ByVal GridAll_Row_STS As Boolean)
        ...
        Else
            ...
            txt_SubAmount.Text = Val(txt_NoofItems.Text) * Val(txt_Rate.Text)
            txt_DiscountAmount.Text = Format(Val(txt_SubAmount.Text) * Val(txt_DiscPerc.Text) / 100, "#########0.00")
            txt_TaxAmount.Text = Format((Val(txt_SubAmount.Text) - Val(txt_DiscountAmount.Text)) * Val(txt_TaxRate.Text) / 100, "#########0.00")
            txt_Amount.Text = Format(Val(txt_SubAmount.Text) + Val(txt_TaxAmount.Text), "########0.00")
    ...
    Private Sub NetAmount_Calculation()
        ...
        NtAmt = Format(Val(txt_Freight.Text) + Val(txt_SubTotal.Text) - Val(txt_TotalDiscAmount.Text) - Val(txt_CashDiscAmount.Text) + Val(txt_AddLessAmount.Text) + Val(lbl_CGstAmount.Text) + Val(lbl_SGstAmount.Text) + Val(lbl_IGstAmount.Text), "######.00")
        ...
        '  NtAmt = Val(txt_GrossAmount.Text) - Val(lbl_CashDiscAmount.Text) + Val(lbl_TaxAmount.Text) + Val(lbl_CGstAmount.Text) + Val(lbl_SGstAmount.Text) + Val(lbl_IGstAmount.Text) + Val(txt_Freight.Text) + Val(txt_AddLess.Text)
                ...
                If .Rows.Count > 0 Then
                    For i = 0 To .Rows.Count - 1
                        ...
                        If Trim(.Rows(i).Cells(1).Value) <> "" And Val(.Rows(i).Cells(17).Value) <> 0 Then
                            '  If Trim(.Rows(i).Cells(1).Value) <> "" And Trim(.Rows(i).CELLS(16).Value) <> "" And Val(.Rows(i).CELLS(17).Value) <> 0 Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    For i = 0 To dt.Rows.Count - 1
                        ...
                        If InterStateStatus = True Then
                            ...
                            If Val(.Rows(n).Cells(7).Value) = 0 Then .Rows(n).Cells(8).Value = ""
    ...
    Private Sub cbo_OrderNo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_OrderNo.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Order_Program_head", "Ordercode_forSelection", "not Ledger_IdNo = " & Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_Ledger.Text), "")
    End Sub
    ...
    Private Sub cbo_OrderNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_OrderNo.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_OrderNo, txt_Description, cbo_Unit, "Order_Program_head", "Ordercode_forSelection", "not Ledger_IdNo = " & Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_Ledger.Text), "")
    ...
    Private Sub cbo_OrderNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_OrderNo.KeyPress
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_OrderNo, cbo_Unit, "Order_Program_head", "Ordercode_forSelection", " not Ledger_IdNo = " & Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_Ledger.Text), "")
    ...
    Private Sub cbo_Order_No_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)

    End Sub

    Private Sub txt_DateTime_Of_Supply_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_DateTime_Of_Supply.TextChanged
    ...
    Public Sub FindMatchingBill()
        ...
        Try
            ...
            If Insert_Entry = True Or New_Entry = False Then
                NewCode = Pk_Condition + Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_PurchaseNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
