Public Class Purchase_Garments2
    ...
    Private Pk_Condition As String = "GMPUR-"
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name as LedgerName from Purchase_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo where a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            da1.Fill(dt1)
            ...
            If dt1.Rows.Count > 0 Then
                ...
                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Size_Name,D.Colour_Name ,e.Gender_Name,f.Design_Name,g.Sleeve_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Size_Head c on a.Size_idno = c.Size_idno LEFT OUTER JOIN Colour_Head d on a.Colour_idno = d.Colour_idno LEFT OUTER JOIN Gender_Head e on a.Gender_idno = e.Gender_idno LEFT OUTER JOIN Design_Head f on a.Design_idno = f.Design_idno LEFT OUTER JOIN Sleeve_Head g on a.Sleeve_idno = g.Sleeve_idno where a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' Order by a.sl_no", con)
                dt2 = New DataTable
                da2.Fill(dt2)
            ...
            With dgv_Details
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Val(dgv_Details.Rows(i).Cells(8).Value) <> 0 Then
                        ...
                        cmd.CommandText = "Insert into Purchase_Details ( Purchase_Code,            Company_IdNo          ,              Purchase_No             ,                                              for_OrderBy                   , Purchase_Date,          Ledger_IdNo    ,            SL_No     ,          Item_IdNo      ,          Unit_IdNo      ,            Size_IdNo   ,         Colour_Idno       ,    Design_IdNo       ,      Gender_IdNo          ,                          Sleeve_IdNo           ,        Tax_Perc                             ,     Noof_Items                        ,                 Tax_Rate                     ,                 Rate                   ,                    Amount           ) " & _
    ...
    Private Sub txt_Weight_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_Weight.KeyDown
        ...
        If e.KeyCode = 38 Then e.Handled = True : txt_Noof_Bags.Focus()
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.* from Purchase_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo INNER JOIN Company_Head c ON a.Company_IdNo = c.Company_IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            prn_HdDt = New DataTable
            da1.Fill(prn_HdDt)

            If prn_HdDt.Rows.Count > 0 Then

                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Size_Name, d.Colour_Name   , e.Design_Name  , f.Gender_Name  ,  g.Sleeve_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Size_Head c on a.size_idno = c.size_idno LEFT OUTER JOIN Colour_Head d ON a.Colour_IdNo = d.Colour_IdNo LEFT OUTER JOIN Design_Head e ON a.Design_IdNo = e.design_IdNo LEFT OUTER JOIN Gender_Head f ON a.Gender_IdNo = f.Gender_IdNo LEFT OUTER JOIN Sleeve_Head g ON a.Sleeve_IdNo = g.Sleeve_IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' Order by a.sl_no", con)
    ...
    Private Sub Printing_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_Name, c.Size_Name , d.Colour_Name   , e.Design_Name  , f.Gender_Name  ,  g.Sleeve_Name from Purchase_Details a INNER JOIN Item_Head b on a.Item_IdNo = b.Item_IdNo LEFT OUTER JOIN Size_Head c on a.size_idno = c.size_idno LEFT OUTER JOIN Colour_Head d ON a.Colour_IdNo = d.Colour_IdNo LEFT OUTER JOIN Design_Head e ON a.Design_IdNo = e.design_IdNo LEFT OUTER JOIN Gender_Head f ON a.Gender_IdNo = f.Gender_IdNo LEFT OUTER JOIN Sleeve_Head g ON a.Sleeve_IdNo = g.Sleeve_IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Purchase_Code = '" & Trim(Pk_Condition) & Trim(EntryCode) & "' Order by a.sl_no", con)
        da2.Fill(dt2)
