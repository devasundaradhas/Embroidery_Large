...
Public Class SalesEntry_Simple6
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                '---Advance
                da3 = New SqlClient.SqlDataAdapter("select sum(a.amount) as AdvAmount  from Voucher_Bill_Details a LEFT OUTER JOIN Sales_Head b ON a.entry_identification = 'SALES-'+b.Sales_Code  where a.Voucher_Bill_Code = '" & Trim(dt1.Rows(0).Item("Voucher_Bill_Code").ToString) & "' and b.company_idno = " & Str(Val(lbl_Company.Tag)) & " and b.Sales_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' ", con)
                dt3 = New DataTable
                da3.Fill(dt3)
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.CommandText = "update voucher_bill_head set credit_amount = a.credit_amount - b.amount from voucher_bill_head a, voucher_bill_details b where b.company_idno = " & Str(lbl_Company.Tag) & " and b.entry_identification = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and b.crdr_type = 'CR' and a.voucher_bill_code = b.voucher_bill_code and a.company_idno = b.company_idno"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "update voucher_bill_head set debit_amount = a.debit_amount - b.amount from voucher_bill_head a, voucher_bill_details b where b.company_idno = " & Str(lbl_Company.Tag) & " and b.entry_identification = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and b.crdr_type = 'DR' and a.voucher_bill_code = b.voucher_bill_code and a.company_idno = b.company_idno"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "delete from voucher_bill_details where company_idno = " & Str(lbl_Company.Tag) & " and   entry_identification = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
                    ...
                    If Val(.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        If itm_id <> 0 Then
                            ...
                            cmd.CommandText = "Insert into Item_Processing_Details (    Reference_Code    ,             Company_IdNo         ,            Reference_No           ,          for_OrderBy       , Reference_Date,          Ledger_IdNo    ,           Party_Bill_No           ,            SL_No     ,          Item_IdNo      ,           Unit_IdNo     ,                         Quantity               ) " & _
                                                    " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(vforOrdby)) & ",    @SalesDate , " & Str(Val(led_id)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(Sno)) & ", " & Str(Val(itm_id)) & ", " & Str(Val(unt_id)) & ", " & Str(-1 * Val(.Rows(i).Cells(5).Value)) & " )"
                            cmd.ExecuteNonQuery()
            ...
            If Val(txt_Advance.Text) <> 0 Then
                Nr = 0
                cmd.CommandText = "Insert into Voucher_Details ( Voucher_Code                       ,      For_OrderByCode       ,         Company_IdNo             ,           Voucher_No              ,            For_OrderBy     , Voucher_Type, Voucher_Date, SL_No,          Ledger_IdNo     ,             Voucher_Amount          ,         Narration                             ,             Year_For_Report                               ,           Entry_Identification               ) " & _
                                  " Values             ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(vforOrdby)) & ", " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(vforOrdby)) & ", 'Sales'     ,   @SalesDate,   7  , " & Str(Val(led_id)) & ", " & Str(Val(txt_Advance.Text)) & ", 'Bill No . : " & Trim(lbl_InvoiceNo.Text) & "', " & Str(Val(Year(Common_Procedures.Company_FromDate))) & ", '" & Trim(Pk_Condition) & Trim(NewCode) & "' ) "
                Nr = cmd.ExecuteNonQuery()
            End If
            ...
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), dtp_Date.Text, led_id, Trim(lbl_InvoiceNo.Text), 0, CSng(Val(lbl_NetAmount.Text)), "DR", Trim(Pk_Condition) & Trim(NewCode), tr)
            ...
            If Val(txt_Advance.Text) <> 0 Then
                ...
                Vou_Code = Trim(Common_Procedures.get_FieldValue(con, "voucher_bill_head", "Voucher_Bill_Code", "entry_identification = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", lbl_Company.Tag, tr))
                ...
                cmd.CommandText = "Insert into voucher_bill_details (   Voucher_Bill_Code   ,           Company_Idno            , Voucher_Bill_Date,        Ledger_Idno      ,  entry_identification                       ,            Amount                 ,  CrDr_Type                ) " & _
                                                    " Values ( '" & Trim(Vou_Code) & "'     , " & Str(Val(lbl_Company.Tag)) & " ,  @SalesDate      , " & Str(Val(led_id)) & ", '" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(txt_Advance.Text)) & ", 'CR' ) "
                Nr = cmd.ExecuteNonQuery()


                Nr = 0
                cmd.CommandText = "update voucher_bill_head set Credit_amount = Credit_amount + " & Str(Val(txt_Advance.Text)) & " where company_idno = " & Str(lbl_Company.Tag) & " and ledger_idno = " & Str(led_id) & " and Voucher_Bill_Code ='" & Trim(Vou_Code) & "'   and  entry_identification = '" & Trim(Pk_Condition) & Trim(NewCode) & "'" ' and crdr_type = 'DR'"
                Nr = cmd.ExecuteNonQuery()
...
LOOP1:
    ...
    Private Sub Get_OrderDetails()
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select a.* ,b.* ,tP.*,IH.*,UH.Unit_name ,VBH.* from Sales_Head a INNER JOIN Sales_Details B ON A.Sales_Code = B.Sales_Code LEFT OUTER JOIN Voucher_Bill_Head VBH ON VBH.Entry_Identification = 'SALES-' + A.Sales_Code  LEFT OUTER JOIN Ledger_Head tP on b.Ledger_IdNo = tP.Ledger_IdNo LEFT OUTER JOIN Item_Head IH ON b.Item_IdNo = IH.Item_IdNo LEFT OUTER JOIN Unit_Head UH ON b.Unit_IdNo = UH.Unit_IdNo  where a.company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Sales_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' " & IIf(Trim(Condt) <> "", " and ", "") & Condt & " Order by a.for_orderby, a.Sales_No", con)
            dt = New DataTable
            da.Fill(dt)
            ...
            If dt.Rows.Count > 0 Then
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    '---Advance
                    da1 = New SqlClient.SqlDataAdapter("select sum(a.amount) as AdvAmount  from Voucher_Bill_Details a LEFT OUTER JOIN Sales_Head b ON a.entry_identification = 'SALES-'+b.Sales_Code  where a.Voucher_Bill_Code = '" & Trim(dt.Rows(i).Item("Voucher_Bill_Code").ToString) & "' and b.company_idno = " & Str(Val(lbl_Company.Tag)) & " and b.Sales_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' ", con)
                    dt1 = New DataTable
                    da1.Fill(dt1)
                    If dt1.Rows.Count > 0 Then
                    ...
                    da1 = New SqlClient.SqlDataAdapter("select sum(a.amount) AS RecAmount  from Voucher_Bill_Details a  where a.entry_identification <> 'SALES-'+ '" & Trim(dt.Rows(i).Item("Sales_CODE").ToString) & " ' and  a.Voucher_Bill_Code ='" & Trim(dt.Rows(i).Item("Voucher_Bill_Code").ToString) & "'  AND a.entry_identification like '%/" & Trim(Common_Procedures.FnYearCode) & "' ", con)
                    dt1 = New DataTable
                    da1.Fill(dt1)
                    If dt1.Rows.Count > 0 Then
